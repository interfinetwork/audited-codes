...
contract BNBMDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BNBM_Dividend_Tracker", "BNBM_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BNBM_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BNBM_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BNBM contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BNBM_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BNBM_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BNBM is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BNBMDividendTracker public dividendTracker;
    ...
    function setSellFee(uint256 _onSellbnbRewardFee, uint256 _onSellliquidityFee, uint256 _onSellMarketingFee) public onlyOwner {
        
        onSelltotalFees = _onSellbnbRewardFee;
        onSellliquidityFee = _onSellliquidityFee;
        onSellmarketingFee = _onSellMarketingFee;

        onSelltotalFees = onSellBNBRewardsFee.add(onSellliquidityFee).add(onSellmarketingFee);
    ...
    constructor() ERC20("BNB Masters", "BNBM") {
        BNBRewardsFee = 7;
        liquidityFee = 1;
        marketingFee = 2;

        onSellBNBRewardsFee = 15;
        onSellliquidityFee = 1;
        onSellmarketingFee = 2;

        marketingWallet = payable(0x49478D67E5f9D327dbEf99728c01a8F02F1A998E);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee);

        onSelltotalFees = onSellBNBRewardsFee.add(onSellliquidityFee).add(onSellmarketingFee);

    	dividendTracker = new BNBMDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BNBM: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BNBM: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BNBM: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BNBM: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BNBM: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BNBM: Cannot update gasForProcessing to same value");
