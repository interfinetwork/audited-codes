...
// File contracts/IconicNFT721a.sol
...
contract IconicNFT721a is ERC721A, ReentrancyGuard{
  ...
  uint256 mintPrice;
  uint64 mintSize;
  uint64 maxMintsPerAddress = 3;
  bytes32 allowlistMerkleRoot = '';
  address payable iconicAddress;
  address admin;

  constructor(string memory name_, string memory symbol_, string memory baseURI_,
        bytes32 merkleRoot_, address iconicAddress_, uint256 mintPrice_, uint64 mintSize_)
        ERC721A(name_, symbol_) {
        require(msg.sender != iconicAddress_, "Error");
        admin = msg.sender;
  ...
  function setAllowListActive(bool active) public {
  ...
  function setBurningActive(bool active) public {
    require(msg.sender == admin, "Not Authorized");
  ...
  function allowlistMint(bytes32[] calldata merkleProof, uint16 amount) external payable nonReentrant() {
        ...
        require(allowlistActive, "Allow list minting not enabled");
        require(msg.value == mintPrice * amount, "Incorrect amount of ether sent");
        require(_totalMinted() + amount <= mintSize, "Can not mint that many");

        uint64 mintCountBySender = _getAux(_msgSender()) + amount;
        require(mintCountBySender <= maxMintsPerAddress, "Exceeds allowed mints");
        ...
        _setAux(_msgSender(), mintCountBySender);
    }
    ...
    function mint(uint16 amount) external payable nonReentrant() {
        ...
        require(balanceOf(msg.sender) + amount <= maxMintsPerAddress, "Exceeds maximum allowed mints");
    ...
    function burn(uint256 tokenId) public {
        require(burningActive, "Burning is not enabled");
        _burn(tokenId, true);
    }
}
