...
contract BitValley is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BitValleyDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BitValley", "BitV") {
        marketingWallet = payable(0x07aE091670c8bf17d6010EbABC68C888Eae76899);
        ...
        dividendTracker = new BitValleyDividendTracker(
            payable(this),
            router,
    ...
    function updateFees(
        ...
        require(liquidityBuy <= 5 && liquiditySell <= 5, "Liquidity fees can't exceed 5%");
        require(deadBuy <= 3 && deadSell <= 3, "Burn fees can't exceed 3%");
        require(RewardsBuy <= 7 && RewardsSell <= 7, "Reward fees cannot exceed 7%");
        require(marketingBuy <= 10 && marketingSell <= 10, "Marketing fees can't exceed 10%");
        buyDeadFees = deadBuy;
...
contract BitValleyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BitValley public BitValleyContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BitValleyContract = BitValley(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BitValley_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BitValley_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BitValley contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BitValleyContract);

            uint256 prevBalance = BitValleyContract.balanceOf(address(this));
            ...
            {
                uint256 received = BitValleyContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BitValleyContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
