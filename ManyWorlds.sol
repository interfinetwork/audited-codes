...
contract HolderRewarderDistributor is IHolderRewarderDistributor {
  ...
  uint256 private _rewardTermDuration = (7 days);
...
contract ManyWorlds is IBEP20, Authorization {
  ...
  address public _operationsAddress = (0xDBce0f6EF63d1160F04944983aeFbCfcf4a0c05E);
  address public _lottoAddress = (0x981ba7Baa33Fb9B6F4A1a0808869C535C975C57F);
  IBEP20 private BUSD = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
  address private WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
  ...
  */
  function checkFounders() internal {
    if (_lastLockDate.add(90 days) < getBlockTimestamp())
    {
      for (uint256 i = 0; i < _founders.length; i++)
      {
        address founder = _founders[i];
