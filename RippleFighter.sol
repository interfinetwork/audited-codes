...
contract RippleFighter is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("RippleFighter", "RF"){
        ...
        totalFees = rewardsFee + marketingFee + liquidityFee + buybackFee + gameDevFee;

    	dividendTracker = new DividendTracker();
    	
    	marketingWallet = address(owner()); // update to Marketing Wallet if set to Owner... can always be changed later too.
        gameDevWallet = address(owner()); // update to Marketing Wallet if set to Owner... can always be changed later too.
    ...
    function updateFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _gameDevFee, uint256 _feeDivisor) external onlyOwner {
        require(_feeDivisor == 100 || _feeDivisor == 1000, "Fee divisor must be 100 or 1000");
        if(_feeDivisor == 100){
            require(_marketingFee <= 5, "Marketing may not be set higher than 5%");
            ...
            require(_gameDevFee <= 2, "game dev fee may not be set higher than 2%");
        }
        if(_feeDivisor == 1000){
            require(_marketingFee <= 50, "Marketing may not be set higher than 5%");
            ...
            require(_gameDevFee <= 20, "game dev fee may not be set higher than 2%");
        }
        marketingFee = _marketingFee;
        rewardsFee = _rewardsFee;
        liquidityFee = _liquidityFee;
        ...
        totalFees = marketingFee + rewardsFee + liquidityFee + buybackFee + gameDevFee;
        if(_feeDivisor == 100){
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(boughtEarly[from] && earlyBuyPenaltyEnd > block.timestamp && !automatedMarketMakerPairs[from]) {
        	    super._transfer(from, address(0xdead), fees * 3);
    ...
    function swapBack(uint256 contractTokenBalance) internal {
        ...
        uint256 amountBNBGameDev = amountBNB.mul(gameDevFee).div(totalBNBFee);
        ...
        (success,) = address(gameDevWallet).call{value: amountBNBGameDev}("");
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 100*1e18; //must hold 100 tokens to receive dividends
    }
