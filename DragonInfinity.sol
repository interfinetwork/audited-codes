...
contract DragonInfinity is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private _isExcludedFromMaxSellToken;
	...
	address private _marketingWalletAddress = 0xFf58567B8B297A9980698D0B6f1064A668290877;	
    address private _developmentWalletAddress = 0xe967d91d27F52bC11105795Fa9eeb6bDc0933527;		
     address private _gameWalletAddress = 0x84A904045e5A76ca25e8F0F7d8C98953B174E7ee;
    ...
    uint256 private numTokensSellToAddToLiquidity = 1000000000 * 10**9;
     uint256 public _maxwalletamount = 15000000000 * 10**9;
      uint256 public _maxsellamount = 10000000000 * 10**9;
	...
	constructor() {
        ...
        _isExcluded[_developmentWalletAddress] = true;
         _isExcludedFromFee[_gameWalletAddress] = true;
         _isExcluded[_gameWalletAddress] = true;
	...
	function setExcludeFromMaxWalletToken(address _address, bool value) public onlyOwner {
        _isExcludedFromMaxWalletToken[_address] = value;
    }
	
	receive() external payable {}

	function _txToDevelopmentWallet(address sender, uint256 totalFee) internal {
        uint256 developmentAmount = totalFee.div(11); 
        developmentAmount = tokenFromReflection(developmentAmount);
        _tOwned[_developmentWalletAddress] = _tOwned[_developmentWalletAddress].add(developmentAmount);
        
       
        emit Transfer(sender, _developmentWalletAddress, developmentAmount);
    }
    
    function _txToMarketingWallet(address sender, uint256 totalFee) internal {
        uint256 marketingAmount = totalFee.div(11); 
        marketingAmount = tokenFromReflection(marketingAmount);
        _tOwned[_marketingWalletAddress] = _tOwned[_marketingWalletAddress].add(marketingAmount);
        
       
        emit Transfer(sender, _marketingWalletAddress, marketingAmount);
    }

    function _txTogameWallet(address sender, uint256 totalFee) internal {
        uint256 gameAmount = totalFee.div(11); 
        gameAmount = tokenFromReflection(gameAmount);
        _tOwned[_gameWalletAddress] = _tOwned[_gameWalletAddress].add(gameAmount);
        
       
        emit Transfer(sender, _gameWalletAddress, gameAmount);
    }
	
	function _reflectFee(uint256 rFee, uint256 tFee) private {
		uint256 actualRFee = rFee.div(2); //2%

		_rTotal = _rTotal.sub(actualRFee);
	...
	function _transfer(
        ...
        if(to != owner() && to != address(this) && to != address(0x000000000000000000000000000000000000dEaD) && to != uniswapV2Pair && to != _developmentWalletAddress && to != _gameWalletAddress && to != _marketingWalletAddress){
            uint256 heldTokens = balanceOf(to);
            require((heldTokens + amount) <= _maxwalletamount);
        }
	...
	function _transferStandard(
		...
		_txToMarketingWallet(sender, rFee.mul(5));
        _txToDevelopmentWallet(sender, rFee.mul(3));
        _txTogameWallet(sender, rFee.mul(3));
