...
// File: contracts/PureBlood.sol

pragma solidity ^0.8.10;

contract PureBloodToken is Context, IERC20, Ownable {
    ...
    string private _name = "PureBlood Token";
    string private _symbol = "$PUREBLOOD";
    ...
    address public _marketingWallet =
        payable(address(0x6a91a3bffB2D65b5dcCaD661904B5b793305e718));
    address public _buybackWallet =
        payable(address(0xB98f17d0e13465DF522BcFE1EdE08EbC460A975a));
    address public _charityWallet = payable(address(0x814D2ba49F1C5DE0162b53F30f3E3b99D6E33D11));
    address public _treasuryWallet = payable(address(0xe51Af42491a2A843F6342b43C5c689D10FdFc983));
    ...
    constructor() {
        ...
        buyFee.taxFee = 2;
        buyFee.charityFee = 1;
        buyFee.treasuryFee = 2;

        sellFee.liquidityFee = 3;
        sellFee.marketingFee = 2;
        sellFee.buybackFee = 3;
        sellFee.taxFee = 2;
        sellFee.charityFee = 2;
        sellFee.treasuryFee = 3;

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            0x10ED43C718714eb63d5aA57B78B54704E256024E
        ); //0xD99D1c33F9fC3444f8101754aBC46c52416550D1  - Testnet Router PCS
           //0x10ED43C718714eb63d5aA57B78B54704E256024E  - Mainnet Router PCS
  ...
  tradingStartBlock = block.number;
        ...
        )
    {
        ...
        uint256 tWallet = calculateBuyBackFee(tAmount) +
    ...
    function _takeLiquidityAndCharity(uint256 tLiquidity, uint256 tCharity)
        private
    {
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
    ...
    function _takeBuybackMarketingAndTreasury(
        uint256 tBuyback,
        uint256 tMarketing,
        uint256 tTreasury
    ) private {
    ...
    function setSell() private {
        ...
        _charityFee = sellFee.charityFee;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 forLiquidity = contractTokenBalance
                ...
                .div(buyFee.marketingFee + sellFee.marketingFee + buyFee.liquidityFee + sellFee.liquidityFee + buyFee.buybackFee + sellFee.buybackFee + buyFee.treasuryFee + sellFee.treasuryFee);

            uint256 forBuyback = contractTokenBalance
                .mul(buyFee.buybackFee + sellFee.buybackFee)
                .div(buyFee.marketingFee + sellFee.marketingFee + buyFee.liquidityFee + sellFee.liquidityFee + buyFee.buybackFee + sellFee.buybackFee + buyFee.treasuryFee + sellFee.treasuryFee);

            uint256 walletFees = contractTokenBalance - forLiquidity - forBuyback;    
                
            

           swapAndLiquify(forLiquidity); // Add liquidity
           transfer(_buybackWallet, forBuyback); // send tokens to buybackWallet
           swapAndSendToWallets(walletFees
            ); // sending BNB to marketing and Treasury Wallet
    ...
    function swapAndSendToWallets(uint256 amount)
        private
        lockTheSwap
    {
        uint256 initialBalance = address(this).balance;
        ...
        uint256 forMarketing = newBalance
            ...
            .div(buyFee.treasuryFee + sellFee.treasuryFee + buyFee.marketingFee + sellFee.marketingFee);
       
        uint256 forTreasury = newBalance - forMarketing;
            
        payable(_marketingWallet).transfer(forMarketing);
        payable(_treasuryWallet).transfer(forTreasury);
    ...
    function _transferStandard(
        ...
        _takeLiquidityAndCharity(tLiquidity, calculateCharityFee(tAmount));
        _takeBuybackMarketingAndTreasury(
