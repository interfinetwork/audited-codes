...
contract EverWealth is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    EverWealthDividendTracker public dividendTracker;
    ...
    constructor() BEP20("EverWealth", "EWEALTH") {
        marketingWallet = payable(0xBB469de0C0ca21B1AA1500b11caaC2043cc4e846); 
        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;


        buyDeadFees = 100;
        sellDeadFees = 100;
        buyMarketingFees = 200;
        sellMarketingFees = 200;
        buyLiquidityFee = 100;
        ...
        dividendTracker = new EverWealthDividendTracker(
            payable(this),
            router,
    ...
    function updateFees(
        ...
        require(liquidityBuy <= 300 && liquidityBuy >= 25 
                && liquiditySell <= 300 && liquiditySell >= 25, 
                "Liquidity fees cannot be lower than 0.25% or higher than 3%");
        require(deadBuy <= 300 && deadSell <= 300, "Burn fees can't exceed 3%");
        require(RewardsBuy <= 2600 && RewardsBuy >= 25 
                && RewardsSell <= 2600 && RewardsSell >= 25, 
                "Reward fees cannot be lower than 0.25% or higher than 26%");
        require(marketingBuy <= 300 && marketingSell <= 300, "Marketing fees can't exceed 3%");
        buyDeadFees = deadBuy;
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 burntokens = amount.mul(deadFees).div(10000);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividendsFromBuy = address(this)
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee.add(buyMarketingFees));
        ...
        uint256 dividendsFromSell = address(this)
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee.add(sellMarketingFees));
        ...
        uint256 _marketTotal = sellMarketingFees.add(buyMarketingFees);
...
contract EverWealthDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    EverWealth public EverWealthContract;
    ...
    constructor(
        ...
        defaultToken = token;
        EverWealthContract = EverWealth(mainContract);
    ...
    function _transfer(
        ...
        require(false, "EverWealth_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "EverWealth_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EverWealth contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(EverWealthContract);

            uint256 prevBalance = EverWealthContract.balanceOf(address(this));
            ...
            {
                uint256 received = EverWealthContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    EverWealthContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
