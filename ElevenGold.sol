...
contract YummyToken is Context, IERC20, Ownable {
    ...
    string private _name = "Eleven Gold4";
    string private _symbol = "ELGD4";
    ...
    function excludeFromReward(address account) public onlyOwner() {
        require(account != 0xCDe540d7eAFE93aC5fE6233Bee57E1270D3E330F, 'We can not exclude Pancake router.');
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (recipient == uniswapV2Pair && sender != address(uniswapV2Router)){
            uint256 fees = _DevFee.add(_taxFee);
            require(amount.add(fees) <= _maxTxAmount || isTxLimitExempt[sender], "TX Limit Exceeded");
         }
        ...
        //Temporarily remove fees to transfer to Dev wallet
        _taxFee = 0;
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function AntiSnipedump(bool selling) public view returns (uint256) {
        ...
        if(selling) { return (_DevFee).mul(multiplier); }
        if (firstFewBlocks) {return multiplier.add(1); }
        return _DevFee;
    }
