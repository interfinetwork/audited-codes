...
import "./permabot-gold-imports.sol";
...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "PermaBot Gold";
    string internal constant SYMBOL = "PBTg";
     ...
     */
    uint256 internal constant maxTransactionAmount = 5 * TOTAL_SUPPLY / 100; // 5% of the total supply
     ...
     * See: `Liquifier`
     */
    uint256 internal constant numberOfTokensToSwapToLiquidity = 2 * TOTAL_SUPPLY / 1000; // 0.2% of the total supply
    ...
    address internal charityAddress = 0x872d3287e8cafecbF040673B67DFd6A2F765Cd58;
    address internal marketingAddress = 0x810F059e346af19C98259AC7b48D2E402Db6E352;
    ...
    function _addFees() private {
         ...
         */ 
        _addFee(FeeType.Rfi, 40, address(this) ); //4% holders

        // _addFee(FeeType.Burn, 10, burnAddress );//No burn
    ...
    function _setMarketingAddress(address newMarketingAddress) internal {
...
abstract contract BaseRfiToken is IERC20, IERC20Metadata, Ownable, Presaleable, Tokenomics {
    ...
    //     require(sender != address(0), "PermabotGold: burn from the zero address");
    //     require(sender != address(burnAddress), "PermabotGold: burn from the burn address");

    //     uint256 balance = balanceOf(sender);
    //     require(balance >= amount, "PermabotGold: burn amount exceeds balance");
    ...
    function setMarketingAddress(address newAdress) external onlyOwner { _setMarketingAddress(newAdress); } 
    
    function _approve(address owner, address spender, uint256 amount) internal {
        require(owner != address(0), "PermabotGold: approve from the zero address");
        require(spender != address(0), "PermabotGold: approve to the zero address");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        require(sender != address(0), "PermabotGold: transfer from the zero address");
        require(recipient != address(0), "PermabotGold: transfer to the zero address");
        require(sender != address(burnAddress), "PermabotGold: transfer from the burn address");
...
contract TokenCustom is SafeToken{

    constructor(Env env) SafeToken(env){
        // pre-approve the initial liquidity supply (to safe a bit of time)
        _approve(owner(),address(_router), ~uint256(0));
    }
}

contract PermabotGold is SafeToken{
