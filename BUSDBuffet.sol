...
contract BUSDBuffet is IBEP20, BBFTAuth {
    using SafeMath for uint256;
    ...
    string constant _name = "BUSD Buffet";
    string constant _symbol = "BBFT";
    uint8 constant _decimals = 18;

    uint256 _totalSupply =  1000000000000000000000000000000000; // 1 quadrillion
    uint256 public _maxTxAmount = _totalSupply.div(600); // 0.17% - 1,666,666,666,667
    ...
    uint256 public kitchenFee = 800;
    uint256 totalFee = 2000;
    uint256 public feeDenominator = 10000;

    uint256 public BKITBUSDthreshold = 2500000000000000000000; // 2500

    address public autoLiquidityReceiver;
    address public kitchenFeeReceiver;
    ...
    uint256 public swapThreshold = _totalSupply / 2000; // 500b
    bool public inSwap;
    ...
    constructor () BBFTAuth(msg.sender) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(shouldSwapBKITBUSD()) { swapBKITBUSD(); }
    ...
    function shouldSwapBKITBUSD() internal view returns (bool) {
        return msg.sender != pancakeV2BNBPair
        && IBEP20(BUSD).balanceOf(address(this)) > BKITBUSDthreshold;
    }

    function swapBKITBUSD() internal {
    ...
    function swapBKITBUSDManual() external authorizedFor(Permission.AdjustContractVariables) {
        uint256 sendBUSDAmount = IBEP20(BUSD).balanceOf(address(this));
        IBEP20(BUSD).transfer(address(distributor), sendBUSDAmount);
        distributor.depositBUSD(sendBUSDAmount);
    }
    ...
    function swapBack() internal swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 amountBNBKitchen = amountBNB.mul(kitchenFee).div(totalBNBFee);

            try distributor.deposit{value: amountBNBReflection}() {} catch {}
            (bool kitchenSuccess, ) = payable(kitchenFeeReceiver).call{value: amountBNBKitchen, gas: 30000}("");
            
            emit KitchenTransfer(kitchenSuccess);
    ...
    function setBKITBUSDthreshold(uint256 amount) external authorizedFor(Permission.AdjustContractVariables) {
        BKITBUSDthreshold = amount;
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _kitchenFee, uint256 _feeDenominator) external authorizedFor(Permission.AdjustContractVariables) {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        kitchenFee = _kitchenFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_kitchenFee);
        feeDenominator = _feeDenominator;
        require(totalFee <= 2500);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _kitchenFeeReceiver) external authorizedFor(Permission.AdjustContractVariables) {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        kitchenFeeReceiver = _kitchenFeeReceiver;
    }
