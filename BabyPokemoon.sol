...
contract BabyPokemoonToken is Context, IERC20, IERC20Metadata, AccessControl {
    ...
    uint256 public antiWhaleTxAmountRate = 10; // 0.1%
    ...
    uint256 public antiBotRate = 10; // 0.1%
    ...
    constructor() {
        ...
        distribution = Distribution(0x82537061FbfBdBE2bD62088cbEEB1808FA6A709d); // need to change
        rewardSwapAddress = 0x59abcd1aDE6E357436a1fCd54861c128D9A1F2E7; // need to change

        router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        busd = 0x55d398326f99059fF775485246999027B3197955; // need to change
    ...
    function maxAntiBotMaxAmount() public view returns (uint256) {
        return _calcFee(totalSupply(), antiWhaleTxAmountRate);
    }
    ...
    function setExcludedFromAntiBot(address _address, bool _isExcludedFromAntiBot) public onlyRole(DEFAULT_ADMIN_ROLE) {
        excludedFromAntiBot[_address] = _isExcludedFromAntiBot;
    }

    function setAntiWhaleTxAmountRate(uint256 _antiWhaleTxAmountRate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_antiWhaleTxAmountRate <= 500 && _antiWhaleTxAmountRate >= 10, "ERC20: invalid _antiWhaleTxAmountRate");
    ...
    function setAntiBotRate(uint256 _antiBotRate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_antiBotRate <= 500 && _antiBotRate >= 10, "ERC20: invalid _antiBotRate");
        antiBotRate = _antiBotRate;
    }

    function setAntiBotEnabled(bool _antiBotEnabled) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function updateBuyFees(uint256 _burnFeeBuyRate, uint256 _liquidityFeeBuyRate, uint256 _swapFeeBuyRate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_burnFeeBuyRate.add(_liquidityFeeBuyRate).add(_swapFeeBuyRate) <= 3000);
    ...
    function updateSellFees(uint256 _burnFeeSellRate, uint256 _liquidityFeeSellRate, uint256 _swapFeeSellRate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_burnFeeSellRate.add(_liquidityFeeSellRate).add(_swapFeeSellRate) <= 3000);
    ...
    function updateTransferFees(uint256 _burnFeeTransferRate, uint256 _liquidityFeeTransferRate, uint256 _swapFeeTransferRate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_burnFeeTransferRate.add(_liquidityFeeTransferRate).add(_swapFeeTransferRate) <= 3000);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (
            antiBotEnabled &&
            _isBuy(sender, recipient) &&
            !excludedFromAntiBot[recipient]
        ) {
            require(balanceOf(recipient).add(amount) <= maxAntiBotMaxAmount(), "ERC20: balance exceeds the maxAntiBotMaxAmount");
        }
        ...
        if (!inSwap) {
            ...
            if (
                ...
                if (burnFeeRes > 0) {
                    if (burnFeeReceivers.length > 0) {
                        ...
                        _transferAmount(from, deadAddress, burnFeeRes, true);
                    }
                }
            ...
            if (
                ...
                if (
                    ...
                    _transferAmount(rewardSwapAddress, address(this), rewardBuyToSwap, true);
                ...
                if (amountToSwap > 0) {
                    ...
                    if (liquidityFeeOtherHalf > 0 && liquidityFeeHalf > 0) {
                        ...
                        if (liquidityFeeReceivers.length == 1) {
                            _addLiquidity(liquidityFeeOtherHalf, liquidityFeeBusdAmount, liquidityFeeReceivers[0]);
                        } else {
                            _addLiquidity(liquidityFeeOtherHalf, liquidityFeeBusdAmount, address(distribution));
                        }
                    ...
                    if (swapFeeAmount > 0) {
                        uint256 swapFeeBusdAmount = _calcFee(busdBalance, swapFeeAmount.mul(10000).div(amountToSwap));

                        for (uint256 i = 0; i < swapFeeReceivers.length; i++) {
                            distribution.recoverTokensFor(busd, _calcFee(swapFeeBusdAmount, swapFeeReceiversRate[i]), swapFeeReceivers[i]);
                        }
                    }

                    if (rewardBuyToSwap > 0) {
                        uint256 rewardBusdAmount = _calcFee(busdBalance, rewardBuyToSwap.mul(10000).div(amountToSwap));

                        for (uint256 i = 0; i < rewardSwapReceivers.length; i++) {
                            distribution.recoverTokensFor(busd, _calcFee(rewardBusdAmount, rewardSwapReceiversRate[i]), rewardSwapReceivers[i]);
                        }
                    }
