...
contract MetaSnooker is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MetaDividendTracker public dividendTracker;
    ...
    address payable marketingWallet = 0x000000000000000000000000000000000000dEaD; //ToChange
...
constructor() public ERC20("MetaSnooker", "MSR") {
        uint256 _BNBRewardsFee = 0;
        uint256 _liquidityFee = 0;
        uint256 _buyBackFees = 0;
    	...
    	dividendTracker = new MetaDividendTracker();
 
    	liquidityWallet = owner();
 
 
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x000000000000000000000000000000000000dEaD);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        MetaDividendTracker newDividendTracker = MetaDividendTracker(payable(newAddress));
        ...
        if( 
            ...
            !_isExcludedFromFees[to] //no max for those excluded from fees
             ...
             if(!isExcludedFromFees(to)){
                  require(cooldown[to] < block.timestamp, "Cooldown has not been reached.");
...
contract MetaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    address dev = 0x0000000000000000000000000000000000000000; //ToChange
    ...
    constructor() public DividendPayingToken("MetaDividends", "MetaDividends") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 0 * (10**18); //Choose minimal holdings requirements
    }
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "MetaDividends: No transfers allowed");
    }
 
    function withdrawDividend() public override {
        require(false, "MetaDividends: withdrawDividend disabled. Use the 'claim' function on the main TIKI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MetaDividends: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MetaDividends: Cannot update claimWait to same value");
