...
// File contracts/ReflectoRewards.sol
...
contract ReflectoRewards is Ownable {
    ...
    function _updateRewardsPerToken() public {
        ...
        uint256 totalSupply_ = reflectoToken.totalSupply();
    ...
    function updateReflecto(IERC20 _reflectoToken) external onlyOwner {
        reflectoToken = _reflectoToken;
    }
    ...
    function approveReward(IERC20 _rewardToken) public onlyOwner {
        ...
        payable(reflectoAdd).transfer(tempRewardDetails[_rewardToken].fee / 2);
...
contract Reflecto is IBEP20, Auth {
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < v1holders.length; i++) {
            uint256 bHolder = IBEP20(0xEA3C823176D2F6feDC682d3cd9C30115448767b3)
                .balanceOf(v1holders[i]);
            if (bHolder > 0 && !isUserMigrated[v1holders[i]]) {
                ...
                try reflectoRewards._updateUserRewards(v1holders[i]) {} catch {}
                try distributor.setShare(v1holders[i], bHolder) {} catch {}
            }
        }
        return true;
    }
