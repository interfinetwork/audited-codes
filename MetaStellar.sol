...
contract METASTELLAR is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Metastellar";
    ...
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + utilityFee + devFee;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x6e1C1BB2fbEE5DE877baa60dBA2dA8CaCF9a8Ab1;
        devFeeReceiver = 0x277BdadF7A82Ab1a9C5Cac664abfdF748aFF3486;
        utilityFeeReceiver = DEAD;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != utilityFeeReceiver && !isTxLimitExempt[recipient]){
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _devFee, uint256 _utilityFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        devFee = _devFee;
        utilityFee = _utilityFee;
        totalFee = _liquidityFee + _reflectionFee + _marketingFee + _utilityFee + _devFee;
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Fees cannot be more than 49%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _utilityFeeReceiver, address _devFeeReceiver ) external authorized {
