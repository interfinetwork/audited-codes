...
contract USDTDividendTracker is DividendPayingToken  {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address _dividentToken) DividendPayingToken("GSCD_USDT_Dividend_Tracker", "GSCD_USDT_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10**18); //must hold 3,000,000,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "GSCD_USDT_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "GSCD_USDT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Boda contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GSCD_USDT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GSCD_USDT_Dividend_Tracker: Cannot update claimWait to same value");
...
contract GSCD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxBuyTranscationAmount = 11655000000000 * (10**18);
    uint256 public maxSellTransactionAmount = 11655000000000 * (10**18);
    uint256 public maxWalletBalance = 77700000000000 * (10**18);
    uint256 public swapTokensAtAmount = 20 * 10**6 * 10**18;
 
    uint256 private liquidityFee;
    uint256 private previousLiquidityFee;

    uint256 private usdtDividendRewardsFee;
    ...
    uint256 public totalFees = usdtDividendRewardsFee.add(marketingFee).add(liquidityFee);
    
    uint256 public _usdtDividendRewardsFeeBuy = 6;
    uint256 public _marketingFeeBuy = 7;
    uint256 public _LpFeeBuy = 6;
    
    uint256 public _usdtDividendRewardsFeeSell=13;
    uint256 public _LpFeeSell=7;
    uint256 public _marketingFeeSell=7;  
    
 
    uint256 public usdtDividedRewardsInContract;
    ...
    constructor() ERC20("Gumby Social Club", "GSCD") {
        	
    	marketingWallet = 0xcEbFE70322376c6bD2c540E6A3aa40C1f8fb078f; 

    	usdtDividendToken = 0xdAC17F958D2ee523a2206206994597C13D831ec7; 

        usdtDividendTracker = new USDTDividendTracker(usdtDividendToken);
        ...
        */
        _mint(owner(), 77700000000000 * (10**18));
    }
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "GSCD: The marketing wallet is already this address");
    ...
    function setAuthOnDividends(address account) public onlyOwner {
        usdtDividendTracker.setAuth(account);
    }
 
    function setUsdtDividendEnabled(bool _enabled) external onlyOwner {
        require(usdtDividendEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
    ...
    function updateusdtDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(usdtDividendTracker), "GSCD: The dividend tracker already has that address");
 
        USDTDividendTracker newusdtDividendTracker = USDTDividendTracker(payable(newAddress));
 
        require(newusdtDividendTracker.owner() == address(this), "Token: The new dividend tracker must be owned by the Boda token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "Token: Account is already exluded from fees");
	...
	function processusdtDividendTracker(uint256 gas) external onlyOwner {
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "Token: Trading has not started yet");
        ...
        if (
            ...
            usdtDividendRewardsFee = _usdtDividendRewardsFeeBuy;
            ...
            usdtDividendRewardsFee = _usdtDividendRewardsFeeSell;
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if (usdtDividendEnabled) {
                uint256 usdtTokens = usdtDividedRewardsInContract;
                swapAndSendUsdtDividends(usdtTokens);
                usdtDividedRewardsInContract = 0;
            }
        ...
        if(takeFee) {
            ...
            uint256 tmpUsdtDividedRewardsInContract;
            uint256 tmpLpRewardInContract;

            tmpMarketingRewardPercent = amount.mul(marketingFee).div(100);
            
             tmpUsdtDividedRewardsInContract = amount.mul(usdtDividendRewardsFee).div(100);
            tmpLpRewardInContract = amount.mul(liquidityFee).div(100);

            fees = tmpMarketingRewardPercent.add(tmpUsdtDividedRewardsInContract)
                            .add(tmpLpRewardInContract);

            usdtDividedRewardsInContract = usdtDividedRewardsInContract.add(tmpUsdtDividedRewardsInContract);
