...
contract GEM is ERC20, Ownable {
    ...
    uint256 public _maxHavAmount = 100000 * 10**18;
    uint256 public _leftAmount = 100000 * 10**18;
    uint256 public _sellLeftCount = 1 * 10**16;
    ...
    constructor(
        ...
        swapTokensAtAmount = totalSupply.mul(30).div(10**5); 
    ...
    function setMaxTxAmount(uint256 amount) public onlyOwner {
        ...
        function setMaxHavAmount(uint256 amount) public onlyOwner {
        _maxHavAmount = amount;
    }


    function setLeftAmount(uint256 amount) public onlyOwner {
        _leftAmount = amount;
    }

      function setSellLeftCount (uint256 amount) public onlyOwner {
        _sellLeftCount = amount;
    }
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] &&  !_isExcludedFromFees[to] && from != address(dividendTracker) && to != address(dividendTracker)) {
            ...
            if (
                ...
                !automatedMarketMakerPairs[to]) || (!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to])
            ) {
             ...
             if (
                ...
                uint256 leftBalance =  balanceOf(from);
                 ...
                 require(
                    leftBalance >= amount.add(_sellLeftCount),
                    "You can't sell all of them short."
                );
        ...
        if(takeFee) {
            ...
            uint256 MFee;
            uint256 DFee;
            uint256  leftCount = totalSupply().sub(balanceOf(deadWallet));
