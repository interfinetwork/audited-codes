...
contract CryptoPupper is safeguarded, resourceful, friendly
{
    ...
    function decentralize() public pupper // only the Pupper can call this function, it provides level 1 decentralization
    {
        lvl = true;
    }

    function decentralize_2() public farmpuppy  // only the Farmpuppy can call this function, it provides level 2 decentralization
    {
        lvl_2 = true;
    }

    function decentralize_3() public firepuppy  // only the Firepuppy can call this function, it provides level 3 decentralization
    {
        lvl_3 = true;
    }

    function decentralize_4() public pupper // once level 1 through 3 of decentralization has been activated, the Pupper can activate complete decentralization (level 4). No parameter modification can take place after that. It's like there is police everywhere man, we can't break the rules no more D:
    {
    ...
    uint private Whale_CE = 100;
    uint private Pupper_CE;
    uint private Antirug_CE = 100;
    ...
    {
        Operational = Whale_CE + Antirug_CE + Pupper_CE + Firepuppy_CE + Farmpuppy_CE + Liquiditypuppy_CE;
    }
    ...
    function getLiquiditypuppy_CE() public view returns(uint)  // fetch boi, fetch me that smooth talking Liquidity Puppy!
    {
    ...
    uint private decimals = 14;
    uint private totalSupply = 10000000 * 10 ** 14; // 10 million
    ...
    function swoosh(address from, address to, uint value) public protected // this is the main trade function yo
    {
        ...
        require(((balanceOf(to)+value)*(10000-Operational)/10000) < totalSupply*Whale_CE/10000, "whale throw"); // protection 1
        require(value < (totalSupply*Antirug_CE/10000), "rug throw"); // protection 2
    ...
    uint private lottery_fee; // fee for buying one PAW ticket
    uint private security_paw; // this is a very secure paw

    function set_PAW_fee(uint new_fee) public pupper // new PAW fee
    {
        lottery_fee = new_fee;
    }
    ...
    function draw_PAW(uint joker) public protected pupper // Pupper uses its paw to draw the 5 lucky PAW elements
    {
        require(block.timestamp-PAW_draw_timestamp > 259200); // 3 days must pass
        drawn_PAW[0] = security_paw;
        for(uint i = 1; i < 6; i++)
        {
            drawn_PAW[i] = uint(keccak256(abi.encodePacked(block.difficulty+joker+i, block.timestamp+joker+i*joker, msgSender())))%25;
        }
