...
/*
// VegasOnChain.Finance
// https://t.me/VegasOnChain
// Vegas on Blockchain!
// Huge 50 000$ JackPot to be won straight to your wallet!
// If nobody wins JackPot goes to Buyback & Burn -> ThePump
// Enjoy the never ending FOMO
// KYC + Audit
*/
...
contract VegasOnChain is Context, IERC20, Ownable {
    ...
    // We don't add to liquidity unless we have at least 1 VOC token
    ...
    uint256 private constant JACKPOT_THEPUMP_MIN = 30000 * 10**USDT_DECIMALS;
    uint256 private constant JACKPOT_THEPUMP_MAX = 250000 * 10**USDT_DECIMALS;
    ...
    uint256 private constant JACKPOT_THEPUMP_BUYBACK_MIN = 3000;
    uint256 private constant JACKPOT_THEPUMP_BUYBACK_MAX = 7000;

    string private constant _name = "VegasOnChain";
    string private constant _symbol = "VOC";
    uint8 private constant _decimals = 9;
    uint256 private constant _tTotal = 10000000 * 10**_decimals;

    // Max wallet size initially set to 10%
    uint256 public maxWalletSize = _tTotal.div(10);
    ...
    // 3% jackpot
    uint256 public bJackpotFee = 300;

    // Sell fees
    // 1% liquidity
    uint256 public sLiquidityFee = 100;
    // 4% marketing
    uint256 public sMarketingFee = 400;
    // 3% dev
    uint256 public sDevFee = 300;
    // 5% jackpot
    ...
    // 50% jackpot cashout to last buyer
    ...
    // Jackpot hard limit, BNB value  (~$50k at $400/BNB)
    uint256 public jackpotHardLimit = 125 * 10**(BNB_DECIMALS);
    ...
    uint256 private _lastThePumpCash = 0;
    uint256 private _lastThePumpTokens = 0;
    uint256 private _lastThePumpTimestamp = 0;
    ...
    // Total BNB/VOC collected by various mechanisms (dev, marketing, jackpot)
    ...
    event ThePump(uint256 cashedOut, uint256 tokensOut);
    ...
    function setJackpotHardFeatures(
        ...
        require(
            _jackpotHardBuyback >= JACKPOT_THEPUMP_BUYBACK_MIN &&
                _jackpotHardBuyback <= JACKPOT_THEPUMP_BUYBACK_MAX,
        ...
        require(
            hardLimitUsd >= JACKPOT_THEPUMP_MIN &&
                hardLimitUsd <= JACKPOT_THEPUMP_MAX,
            "Jackpot hard value limit for the big bang needs to be between 30K and 250K USD"
        );
        jackpotHardLimit = _jackpotHardLimit;

        emit ThePumpFeaturesChanged(jackpotHardBuyback, jackpotHardLimit);
    }
        ...
        )
    {
        ...
        // Either 0 or 1+ VOC to prevent PCS errors on liq swap
    ...
    {
        return (_lastThePumpCash, _lastThePumpTokens, _lastThePumpTimestamp);
    }
    ...
    function processThePump() private lockTheSwap {
        ...
        emit ThePump(cashedOut, tokensOut);

        _lastThePumpCash = cashedOut;
        _lastThePumpTokens = tokensOut;
        _lastThePumpTimestamp = block.timestamp;
