...
contract ScowCalculator is Ownable, IScowCalculator, AbstractScow {
    ...
    function _updateUserInfo( address _staker,uint _amount ,bool _add) internal returns ( bool ){
        ...
        if(info.level != 0 && !_add){
            ...
            if(deltaTime < DAY){
                unstakeAmount = unstakeAmount.add(info.unStakeAmount);
            }else{
...
contract CowBondDepository is Ownable {
    ...
    */
    ...
    function deposit( 
        ...
        if(router != address(0)){
            IRouter(router).inviteUser(msg.sender, _inviter, _amount, 3);
        }
     ...
     */
    function _updateBondPrice() internal {
...
contract CowStaking is Ownable {

    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public immutable COW;
    address public immutable sCOW;
    ...
    uint public totalStaked;            // new stake amount
    ...
    /**
        @notice stake COW to enter warmup
        @param _amount uint
        @return bool
     */
    function stake( uint _amount, address _recipient ,address _inviter ) external returns ( bool ) {
        rebase();
        IERC20( COW ).safeTransferFrom( msg.sender, address(this), _amount );

        Claim memory info = warmupInfo[ _recipient ];
        require( !info.lock, "Deposits for account are locked" );
        warmupInfo[ _recipient ] = Claim ({
            deposit: info.deposit.add( _amount ),
            gons: info.gons.add( IsCOW( sCOW ).gonsForBalance( _amount ,_recipient) ),
            expiry: epoch.number.add( warmupPeriod ),
            lock: false
        });
        
        IERC20( sCOW ).safeTransfer( warmupContract, _amount );

        totalStaked = totalStaked.add(_amount);

        if(inviteRouter != address(0)){
            IRouter(inviteRouter).inviteUser(msg.sender, _inviter, _amount, 2);
        }
    ...
    /**
        @notice retrieve sCOW from warmup
        @param _recipient address
     */
    function claim ( address _recipient ) public {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            IWarmup( warmupContract ).retrieve( _recipient, IsCOW( sCOW ).balanceForGons( info.gons ,_recipient) );
        }
    }

    /**
        @notice forfeit sCOW in warmup and retrieve COW
     */
    function forfeit() external {
        Claim memory info = warmupInfo[ msg.sender ];
        delete warmupInfo[ msg.sender ];

        IWarmup( warmupContract ).retrieve( address(this), IsCOW( sCOW ).balanceForGons( info.gons, msg.sender) );
        IERC20( COW ).safeTransfer( msg.sender, info.deposit );
    }
    ...
    function unstake( uint _amount, bool _trigger ) external {
        ...
        IERC20( sCOW ).safeTransferFrom( msg.sender, address(this), _amount );
        IERC20( COW ).safeTransfer( msg.sender, _amount );
        if(inviteRouter != address(0)){
            IRouter(inviteRouter).inviteUser(msg.sender, address(0), _amount, 7);
        }
    }

    function unstakeFromCaller( address _to, uint _amount, bool _trigger ) external onlyCaller {
        if ( _trigger ) {
            rebase();
        }
        IERC20( sCOW ).safeTransferFrom( msg.sender, address(this), _amount );
        IERC20( COW ).safeTransfer(_to, _amount );
        if(inviteRouter != address(0)){
            IRouter(inviteRouter).inviteUser(msg.sender, address(0), _amount, 7);
        }
    }
    ...
    function rebase() public {
        if( epoch.endBlock <= block.number ) {
            IsCOW( sCOW ).rebase( epoch.distribute, epoch.number );
            ...
            uint staked = IsCOW( sCOW ).circulatingSupply();
    ...
    /**
        @notice returns contract COW holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns ( uint ) {
        return IERC20( COW ).balanceOf( address(this) ).add( totalBonus );
    }
    ...
    function giveLockBonus( uint _amount ) external {
        ...
        IERC20( sCOW ).safeTransfer( locker, _amount );
    }
    ...
    function returnLockBonus( uint _amount ) external {
        ...
        IERC20( sCOW ).safeTransferFrom( locker, address(this), _amount );
    }
    ...
    function addCaller(address _newCaller) public onlyManager() returns (bool) {
        require(_newCaller != address(0), "CowStaking: address is zero");
        return EnumerableSet.add(_callers, _newCaller);
    }

    function delCaller(address _delCaller) public onlyManager() returns (bool) {
        require(_delCaller != address(0), "CowStaking: address is zero");
    ...
    function getCaller(uint256 _index) public view returns (address) {
        require(_index <= getCallerLength() - 1, "CowStaking: index out of bounds");
    ...
    modifier onlyCaller() {
        require(isCaller(msg.sender), "CowStaking: not the caller");
        _;
    }
}
// SPDX-License-Identifier: AGPL-3.0-or-later
