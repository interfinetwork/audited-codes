...
contract GoldRushCasino is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GoldRushCasinoDividendTracker public dividendTracker;
    ...
    uint256 public vestingTimer1;
    uint256 public vestingTimer2;
    uint256 public vestingTimer3;
    uint256 public vestingTimer4;
    bool public vestingTimerSet;
    ...
    event updateCharityWallet(address wallet);
    ...
    constructor() ERC20("Gold Rush Casino", "GRC") {
        marketingWallet = payable(0x5D45841905ef02c6D07A9048125B6A3E8c3efE26);
        charityWallet = payable(0x5D45841905ef02c6D07A9048125B6A3E8c3efE26);
        devWallet = payable(0x5D45841905ef02c6D07A9048125B6A3E8c3efE26);
        ...
        dividendTracker = new GoldRushCasinoDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "GoldRushCasinoTRACKER",
            "GRCTRACKER"
        );
        ...
        swapTokensAtAmount = totalTokenSupply / 500000; // 0.005%;
    ...
    function setCharityWallet(address wallet) external onlyOwner {
        ...
        emit updateCharityWallet(wallet);
    }
    ...
    // vest wallets can only vest wallets before trading enabled can't vest wallets after
    ...
    // to set intial vesting timer cant set vesting time after trading is enabled
    function setVestingTime(uint256 initialtimer) public onlyOwner {
        ...
        vestingTimer2 = vestingTimer1 + 2073600;
        vestingTimer3 = vestingTimer2 + 2073600;
        vestingTimer4 = vestingTimer3 + 2073600;
    ...
    // to adjust fees but is limited to a max of 12% buy and 15% sell
    function updateFees(
        uint256 charityBuy,
        uint256 charitySell,
        ...
        require (totalBuyFees <= 12 && totalSellFees <= 15, "cant set fees above buy 12% sell 15% total");
    ...
    function getVestingTime(address account) external view returns (uint256) {
        uint256 timer1 = _holderVestingStarted[account].add(vestingTimer1).sub(block.timestamp);
    ...
    function _transfer(
        ...
        require (!_isVesting[to], "cannot buy tokens while vesting");
        ...
        if (_isVesting[from]) {
            ...
            if (block.timestamp >= _holderVestingStarted[from] + vestingTimer1 
                && block.timestamp < _holderVestingStarted[from] + vestingTimer2 ) {
                    require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(3).div(4), "can only sell 25%");
            }

            if (block.timestamp >= _holderVestingStarted[from] + vestingTimer2
                && block.timestamp < _holderVestingStarted[from] + vestingTimer3) {
                    require(balanceOf(from).sub(amount) >= tokensVesting[from].div(2), "can only sell 50%");
            }

            if (block.timestamp >= _holderVestingStarted[from] + vestingTimer3
                && block.timestamp < _holderVestingStarted[from] + vestingTimer4) {
                    require(balanceOf(from).sub(amount) >= tokensVesting[from].div(4), "can only sell 75%");
            }
    ...
    // can set minimum amount a wallet must hold for auto payout
    ...
    // can set minimum amount a wallet must hold for rewards
    ...
    // can force swapp incase contract is to full and needs emptied
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyCharityFees + buyDevFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellCharityFees + sellDevFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellCharityFees + sellDevFee) 
                                + buyMarketingFees.add(buyCharityFees + buyDevFee);
        ...
        uint256 charityPayout = buyCharityFees.add(sellCharityFees) * feePortions;
        ...
        if (charityPayout > 0) {
            (successOp2, ) = address(charityWallet).call{value: charityPayout}("");
        }

        if (devPayout > 0) {
            (successOp3, ) = address(devWallet).call{value: devPayout}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + charityPayout + devPayout,
            success && successOp1 && successOp2 && successOp3
        );
    }

    // for owner to airdrop wallets native tokens and cant send more than 200 wallets at a time
...
contract GoldRushCasinoDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    GoldRushCasino public GoldRushCasinoContract;
    ...
    constructor(
        ...
        defaultToken = token;
        GoldRushCasinoContract = GoldRushCasino(mainContract);
        minimumTokenBalanceForAutoDividends = 100_000_000000000000000000; // 100,000, 0,001% of supply
    ...
    function _transfer(
        ...
        require(false, "GoldRushCasino_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "GoldRushCasino_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GoldRushCasino contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(GoldRushCasinoContract);

            uint256 prevBalance = GoldRushCasinoContract.balanceOf(address(this));
            ...
            {
                uint256 received = GoldRushCasinoContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GoldRushCasinoContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
