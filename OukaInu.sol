...
/* Token contract */
contract OukaInu is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    // Name and symbol
    string constant _name = "Ouka Inu";
    string constant _symbol = "OUKA";
    ...
    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply * 300 / 10000; // 3% or 3m tokens
    uint256 public _maxSellTxAmount = _totalSupply * 150 / 10000; // 5% or 5M tokens
    ...
    uint256 buybackFeeSell = 2;
    uint256 charityFeeSell = 2;
    ...
    // GREED
    uint256 private GREEDTriggeredAt;
    uint256 private GREEDDuration = 3600;
    ...
    constructor () Auth(msg.sender) {
        ...
        autoLiquidityReceiver = 0xe1e7703841b16d61c23Fdf61d5F5Be6a88c293BB;
        marketingFeeReceiver = 0x79F168512BFC75709cE3919ECcd51DEC48f62502;
        buyBackFeeReceiver = 0x7B607249675C40733E13db4221386822f94B46aa;
        developmentFeeReceiver = 0x0BdFc49D8F691f0d993d013e74F83405b77387b4;
        charityFeeReceiver = 0xeeC03165D878e6634A1Fa2e242760d4fB5711849;
        stakingFeeReceiver = 0x8B543639C657ADf1a5e2F4D839a9F680107faeA6;
    ...
    function inGREEDTime() private returns (bool){
    ...
    // Enable GREED
    function enableGREED(uint256 _seconds) private authorized {
        GREEDTriggeredAt = block.timestamp;
        GREEDDuration = _seconds;
    }

    // Disable the GREED mode
    function disableGREED() private authorized {
        GREEDTriggeredAt = 0;
    }
    ...
    function swapBack() internal swapping {
        ...
        try distributor.deposit{value: amountBNBStaking}() {} catch {}

        payable(marketingFeeReceiver).transfer(amountBNBMarketing);
        payable(buyBackFeeReceiver).transfer(amountBNBBuyBack);
        payable(developmentFeeReceiver).transfer(amountBNBDev);
    ...
    // Set our buy fees
    function setBuyFees(uint256 _stakingFee, uint256 _feeDenominator) external authorized {
        stakingFeeBuy = _stakingFee;
        feeDenominator = _feeDenominator;
    }

    // Set our sell fees
    function setSellFees(uint256 _liquidityFeeSell, uint256 _buybackFeeSell, uint256 _charityFeeSell, uint256 _marketingFeeSell, uint256 _devFeeSell, uint256 _feeDenominator) external authorized {
        liquidityFeeSell = _liquidityFeeSell;
        buybackFeeSell = _buybackFeeSell;
        charityFeeSell = _charityFeeSell;
        marketingFeeSell = _marketingFeeSell;
        devFeeSell = _devFeeSell;
        totalFeeSell = _liquidityFeeSell.add(_buybackFeeSell).add(_charityFeeSell).add(_marketingFeeSell).add(_devFeeSell);
        feeDenominator = _feeDenominator;
    }

    // Set the marketing and liquidity receivers
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _buyBackFeeReceiver, address _developmentFeeReceiver, address _charityFeeReceiver, address _stakingFeeReceiver) external authorized {
