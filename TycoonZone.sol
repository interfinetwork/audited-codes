...
contract TycoonCoin is Context, IERC20, Ownable {
    ...
    address payable public _externalAddress  = payable(0xf669Cfc5f6dd1ffdd1A526B1F2d641C697a447Fa);
    ...
    string private _name    = "Tycoon";
    string private _symbol  = "TYCOON";
    ...
    address public constant BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; //change this
    ...
    constructor () {
        ...
        autoLiquidityReceiver = payable(0x117c60Ab2389CDec53a9C27f81d3B22A4671ebd0);
    ...
    function swapAndLiquify(uint256 tokenBalance) private lockTheSwap {
        ...
        if(_externalFee > 0)
        {
            ...
            swapTokensForBusd(_externalAddress, externalAmount);
            initialBalance = busdContract.balanceOf(address(this));
        }
        ...
        if(_liquidityFee > 0)
        {
            ...
            swapTokensForBusd(address(this), half);
            
            // how much ETH did we just swap into?
            uint256 newBalance = busdContract.balanceOf(address(this)).sub(initialBalance);
    ...
    function addLiquidity(address recipient, uint256 tokenAmount, uint256 busdAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(pancakeRouter), tokenAmount);
        IERC20(BUSD).approve(address(pancakeRouter), busdAmount);
    ...
    function setRouterAddress(address newRouter) public onlyOwner() {
        ...
        address getPair = IPancakeFactory(_newPancakeRouter.factory()).getPair(address(this), BUSD);
        if (getPair == address(0)) {
            pancakePair = IPancakeFactory(_newPancakeRouter.factory()).createPair(address(this), BUSD);
        } else {
            pancakePair = getPair;
        }
