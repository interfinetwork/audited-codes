...
contract SnowFloki is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable private marketingWallet = payable(0xc102C5044D9C4dEb65Bde71E4712323F12491b98); // Marketing Wallet
    address payable private ecosystemWallet = payable(0xfB0d5a3884980ebAe2A217CF21De43521aB0fE1B); // Ecosystem Wallet
    ...
    string private _name = "Snow Floki";
    string private _symbol = "SNOWFLOKI";
    ...
    function multiTransfer(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
        ...
        require(from==msg.sender,"Send tokens from owner only"); 
    ...
    function setTaxesBuy(uint256 _reflectionFee, uint256 _liquidityFee, uint256 _marketingFee,uint256 _devFee,uint256 _ecosystemFee) external onlyOwner {
        ...
        require(totalFee <= 500, "Must keep taxes below 50%");
    }

    function setTaxesSell(uint256 _reflectionFee,uint256 _liquidityFee, uint256 _marketingFee,uint256 _devFee,uint256 _ecosystemFee) external onlyOwner {
        _sellLiquidityFee = _liquidityFee;
        _sellMarketingFee = _marketingFee;
        _sellDevFee = _devFee;
        _sellReflectionFee= _reflectionFee;
        require(_sellLiquidityFee.add(_sellMarketingFee).add(_sellDevFee).add(_sellReflectionFee) <= 500, "Must keep taxes below 50%");
    }
