/**
 *Submitted for verification at cronoscan.com on 2022-04-12
*/

/**

https://t.me/evercroportal
...
contract EverCro is IERC20, Auth {
    
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    //general
    string constant _name = "EverCro";
    string constant _symbol = "$EverCro";
    ...
    uint256 public swapThreshold = 250_000_000 * (10 ** _decimals);
    ...
    constructor () Auth(msg.sender) {
        ...
        //Wallets
        autoLiquidityReceiver = address(0xB8F93Ce7AaC8D35FA1fDaB10f3B33d9DC5Cb3B75);
        marketingWallet = address(0x66528b2d5F178948Dd9a6aEf6592B4d3B749168F);
        buybackWallet = address(0x7b7f89F009147B0717C24038d0a60AD14347C226);
        ...
        totalFee = liquidityFee.add(marketingFee).add(buybackFee).add(rewardsFee);
    ...
    function changeFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee, uint256 newBuybackFee, uint256 newExtraSellFee) external authorized {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isTxLimitExempt[sender] && !isTxLimitExempt[recipient]){
            require(tradingOpen == true,"Trading not started");
            if (recipient != pair) {
                    uint256 contractBalanceRecepient = balanceOf(recipient);
                    require(contractBalanceRecepient + (amount-((amount*totalFee)/100)) <= _walletMax,
    ...
    {        
        ...
        uint256 amountCROBuyback = amountCRO.mul(marketingFee).div(totalCROFee);
        uint256 amountCROMarketing = amountCRO.sub(amountCROLiquidity).sub(amountCROBuyback).sub(amountCROReflection);

        //distribute to wallets and dividend
        try dividendDistributor.deposit{value: amountCROReflection}() {} catch {}
        bool success;
        if (amountCROBuyback > 0) (success, ) = payable(buybackWallet).call{ value: amountCROBuyback, gas: 30000 }("");
    ...
    function withdrawVestedLP() external onlyOwner {
        IERC20(pair).safeTransfer(msg.sender, IERC20(pair).balanceOf(address(this)));
    }

    function RescueCRO(uint256 amount, address payable receiver) external onlyOwner {
