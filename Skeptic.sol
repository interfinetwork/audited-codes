...
contract Skeptic is IERC20, Ownable {
    ...
    // token data
    string private constant _name = 'Skeptic';
    string private constant _symbol = 'SKP';
    ...
    // Holding Exclusion
    uint256 public maxHoldingLimitPercentage;
    ...
    constructor() {
        ...
        // Set Max Holding Data
        maxHoldingLimitPercentage = 2;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // check maximum holding limit
        if (!permissions[recipient].isMaxHoldingsExempt) {
            require(
                balanceOf(recipient) <= _totalSupply.mul(maxHoldingLimitPercentage).div(100),
                'Balance Exceeds Maximum Holdings'
            );
        }
        return true;
    }
    ...
    function setMaxHoldingsExempt(address account, bool isExempt) external onlyOwner {
        permissions[account].isMaxHoldingsExempt = isExempt;
    }

    function setMaxHoldingLimitPercentage(uint256 newMaxHoldingLimitPercentage) external onlyOwner {
        require(
            newMaxHoldingLimitPercentage <= 100 && newMaxHoldingLimitPercentage > 0,
            'Holding Limit Too High'
        );
        maxHoldingLimitPercentage = newMaxHoldingLimitPercentage;
    }
