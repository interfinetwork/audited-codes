...
contract TuneCrueBP is AbstractERC1155Factory, ReentrancyGuard, MessageSigning  {
    ...
    uint32 public freeMintSupply = 300;

    uint16 public maxPerWallet = 1;

    bool public presale = true;
 
    mapping(address => uint256) private _WalletMintCounter;
    ...
    */
    function setNewPrice(uint256 newPrice) external onlyOwner {
        require(price != newPrice, "New Price should be different than the current one");
        price = newPrice;
    }
    ...
    */
    ...
    function setMaxPerWallet(uint16 newMaxPerWallet) external onlyOwner {
        require(maxPerWallet != newMaxPerWallet, "New maxPerWallet should be different than the current one");
    ...
    *
    */
    function whitelistMinting(bytes memory WhitelistSignature) external payable whenNotPaused {
        require (presale, "Presale over!");
        require (isValidSignature(abi.encodePacked(_msgSender(), "presale"), WhitelistSignature), "Wallet not whitelisted");
        require(_WalletMintCounter[msg.sender] + 1 <= maxPerWallet , "max Pass per address exceeded");
    ...
    */
    function _mintPass() private nonReentrant {
        require(totalSupply(1) + 1 <= maxSupply - freeMintSupply, "Purchase: Max supply reached");
        require(msg.value >= price, "Purchase: Incorrect payment");
    ...
    */
     ...
     */
    function getNbrOfMintedToken(address wallet) external view returns (uint256) {
