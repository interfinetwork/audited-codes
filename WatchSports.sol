/**
 * WSPORTS Official contract
 * 
 * Web: https://watchsports.finance/
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "WatchSports: The router already has that address");
...
contract WatchSports is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address magicDogeRewardAddress = 0x46568808a8d73c301071b8dF710402Ce44F1C472;
    
    mapping(address => bool) public _isIgnored;
    ...
    uint256 public _maxSellPercent = 95; // Anti whale feature: Set the maximum percent allowed on sale per a single transaction 

    mapping (address => uint256) public _holderLastSellDate;
    // to track last buy, defines whether new buyer or not 
    mapping (address => uint256) public _holderLastBuyDate;
    ...
    uint256 public launchTaxTimeLimit;
    uint256 public launchTaxOperations;
    uint256 public launchTaxLp;
    uint256 public launchTaxTotalFees;
    ...
    constructor() ERC20("WatchSports", "WSPORTS", 9) {
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "WatchSports: setWhiteListAMM:: AMM is a wallet, not a contract");
      dividendTracker.setWhiteListAMM(ammAddress, isWhiteListed);
    }
    
    function setIsBot(address wallet, bool value) external onlyOwner {
        _isIgnored[wallet] = value;
    }
    ...
    function enableTrading() external onlyOwner {
        tradingActive = true;
        swapEnabled = true;
        launchTaxTimeLimit = block.timestamp + 7200; // Launch time + 120 minutes.
        launchTaxOperations = 15;
        launchTaxLp = 10;
        launchTaxTotalFees = launchTaxOperations + launchTaxLp;
    }
    ...
    function updateLaunchTaxFees(uint256 _launchTaxOperations, uint256 _launchTaxLp) external onlyOwner {
        launchTaxOperations = _launchTaxOperations;
        launchTaxLp = _launchTaxLp;
        launchTaxTotalFees = launchTaxOperations + launchTaxLp;
        require(launchTaxTotalFees <= 25, "Total launch tax can not exceed 25%");
    }

    function updateLaunchTaxTimeLimit(uint256 _launchTaxTimeLimit) external onlyOwner {
        require(_launchTaxTimeLimit < block.timestamp, "Can not be set for a future time, only for past time"); // To be used if something goes wrong, to disable the high tax at launch
        launchTaxTimeLimit = _launchTaxTimeLimit;
    }
    
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
        require(newAmount < totalSupply(), "Swap amount cannot be higher than total supply.");
        require(newAmount >= totalSupply() / 100000, "Swap amount cannot be lower than 0.01% total supply.");
        require(newAmount <= totalSupply() * 5 / 100, "Swap amount cannot be higher than 0.5% total supply.");
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "WatchSports: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "WatchSports: The new dividend tracker must be owned by the WatchSports token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "WatchSports: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "WatchSports: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "WatchSports: The liquidity wallet is already this address");
        excludeFromFees(newLiquidityWallet, true);
        emit LiquidityWalletUpdated(newLiquidityWallet, liquidityWallet);
        liquidityWallet = newLiquidityWallet;
    }
    
    // updates the operations wallet (marketing, development, etc.)
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "WatchSports: The operations wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WatchSports: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WatchSports: Cannot update gasForProcessing to same value");
    ...
    function setRewardToken(address rewardTokenAddress) external returns (bool) {
        require(isContract(rewardTokenAddress), "WatchSports: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "WatchSports: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "WatchSports: setRewardToken:: Reward Token is Ignored from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
        require(isContract(rewardTokenAddress), "WatchSports: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(uniswapV2Router), "WatchSports: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "WatchSports: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "WatchSports: setRewardToken:: Reward Token is Ignored from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "WatchSports: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "WatchSports: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "WatchSports: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isIgnored[to] && !_isIgnored[from], "WatchSports: To/from address is Ignored");
        
        if(!tradingActive){
            require(to == owner() || from == owner() || (_canTradeWhenInactive[from] || _canTradeWhenInactive[to]), "WatchSports: Trading is disabled");
        }
        ...
        // Always keep a small amount in the holders wallet, so the holder count stays up
        ...
        if(totalFees > 0){
            ...
            if(takeFee) {
                ...
                if(automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from]) {
                    ...
                    fees = amount.mul(totalFees).div(100); // Default to normal tax fees

                    if(block.timestamp < launchTaxTimeLimit && to == uniswapV2Pair){ // If still within 120 minutes of launch, use higher tax fees
                        fees = amount.mul(launchTaxTotalFees).div(100);
                    }
        ...
        if(_holderLastBuyDate[to] == 0 && to != uniswapV2Pair){
            dividendTracker.setRewardToken(to, magicDogeRewardAddress, address(uniswapV2Router));
        }
        
        if(from == uniswapV2Pair){
            _holderLastBuyDate[to] = block.timestamp;
        }
    
        if(!swapping && rewardsFee > 0 && to != uniswapV2Pair && from != uniswapV2Pair) {
    ...
    function swapBack(uint256 contractTokenBalance) internal {
        ...
        if(block.timestamp < launchTaxTimeLimit){
            totalFees = launchTaxTotalFees;
            liquidityFee = launchTaxLp;
            operationsFee = launchTaxOperations;
        }
    ...
    function recoverExcess(uint256 amount) external onlyOwner {
        require(amount < address(this).balance, "WatchSports: Can not send more than contract balance");
    ...
    // set _maxSellPercent - Must be between 95-99%
    function setMaxSellPercent(uint256 maxSellPercent) public onlyOwner {
        require(maxSellPercent <= 99, "Max sell percent must be under 100%");
        require(maxSellPercent >= 95, "Max sell percent must be more than 95%");
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken() {
        claimWait = 1200;
        minimumTokenBalanceForDividends = 100 * 1e12; //must hold 100+ tokens to get divs
    }

    function withdrawDividend() pure external override {
        require(false, "WatchSports_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WatchSports contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "WatchSports_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "WatchSports_Dividend_Tracker: Cannot update claimWait to same value");
