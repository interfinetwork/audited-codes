...
contract MetaSantaFloki is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    // use by default gas to process auto-claiming dividends
    ...
    constructor() ERC20("Meta Santa Floki", "MSF") {
        ...
        liquiditySellFee = 1;
        devSellFee = 3;
        totalSellFees = rewardsSellFee + marketingSellFee + liquiditySellFee + devSellFee;
        
        rewardsBuyFee = 4;
        marketingBuyFee = 6;
        liquidityBuyFee = 1;
        devBuyFee = 3;
        totalBuyFees = rewardsBuyFee + marketingBuyFee + liquidityBuyFee + devBuyFee;
        
    	dividendTracker = new DividendTracker();
    	
    	marketingWallet = address(0xe26b7f2Bc7237e9bE77634Df81176EEd53227410); // set as marketing wallet
    	devWallet = address(0x4b7FF36f07972d77d94ddF6fc0FBe7ADba4ade84); // set as dev wallet
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _devFee) external onlyOwner {
        marketingBuyFee = _marketingFee;
        rewardsBuyFee = _rewardsFee;
        liquidityBuyFee = _liquidityFee;
        devBuyFee = _devFee;
        totalBuyFees = marketingBuyFee + rewardsBuyFee + liquidityBuyFee + devBuyFee;
        require(totalBuyFees <= 20, "Must keep fees at 20% or less");
    }
    
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _devFee) external onlyOwner {
        marketingSellFee = _marketingFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
        devSellFee = _devFee;
        totalSellFees = marketingSellFee + rewardsSellFee + liquiditySellFee + devSellFee;
