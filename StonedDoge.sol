...
contract STONEDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("STONED_Dividend_Tracker", "STONED_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000000 * (10**9); // must hold 10m+ tokens 0.001%
    }

    function _transfer(address, address, uint256) internal override pure {
        require(false, "STONED_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "STONED_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main STONED contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "STONED_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "STONED_Dividend_Tracker: Cannot update claimWait to same value");
...
contract StonedDoge is ERC20, Ownable {
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    STONEDDividendTracker public dividendTracker;

    address public _marketingWallet = 0xc757a739C2e5e9Dcc383A62eA2E3778beaa417b0;

    bool public restrictWhales = false;

    uint256 public maxTxAmount = 1000000 * 10**6 * (10**9);
    uint256 public walletMax = 1000000 * 10**6 * (10**9);
    uint256 public swapTokensAtAmount = 250000000 * (10**9);

    uint256 public  BuyRewardsFee = 6;
    ...
    constructor() ERC20("Stoned Doge", "STONED") {

        dividendTracker = new STONEDDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "STONED: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "STONED: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "STONED: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "STONED: Cannot update gasForProcessing to same value");
    ...
    function prepareForLaunch() external onlyOwner {
        updateBuyFees(6,2,2);
        updateSellFees(6,4,4);
        restrictWhales = true;
        cooldownEnabled = false;
        maxTxAmount = 10000 * 10**6 * (10**9);
        walletMax = 10000 * 10**6 * (10**9);
    }
    ...
    function prepareForPresale() external onlyOwner {
        ...
        cooldownEnabled = false;
        maxTxAmount = 1000000 * 10**6 * (10**9);
        walletMax =  1000000 * 10**6 * (10**9);
    }
}