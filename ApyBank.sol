    ...
    contract APYBank is ERC20Detailed, Ownable {
        using SafeMath for uint256;
        using SafeMathInt for int256;

        //Events
        event TimeFupdated(uint256 _timeF);
        event SwapBack(uint256 contractTokenBalance,uint256 amountToLiquify,uint256 amountToRFV,uint256 amountToOperations ,uint256 amountToTreasury);
        ...
        event SellFeesChanged(uint256 _liquidityFee, uint256 _treasuryFee, uint256 _sellFeeRFV, uint256 _burnFee, uint256 _operationsFee);
        event BuyFeesChanged(uint256 _liquidityFee, uint256 _treasuryFee, uint256 _buyFeeRFV, uint256 _referralFee, uint256 _burnFee, uint256 _operationsFee);
        ...
        event SetFeeReceivers(address _liquidityReceiver, address _treasuryReceiver, address _riskFreeValueReceiver, address _operationsFeeReceiver);
        ...
        event ChangedMaxSellTransactionDenom(uint256 _maxSellDenom);
        ...
        event ChangedReferralFeeWithReferral(uint256 _fee);
        ...
        uint256 private rewardYield = 7261139;
        ...
        address private operationsFeeReceiver = 0xa94FCA74301b5760D5d4323db0910aA56b5F4992;
        ...
        uint256 private treasuryFee = 3;
        uint256 private buyFeeRFV = 3;
        uint256 private referralFee = 5;
        uint256 private operationsFee = 2;
        uint256 private burnFee = 1; 

        uint256 private referralFeeWithReferral = 3; 

        //sellFee
        uint256 private sellFeeTreasury = 5;
        uint256 private sellFeeRFV = 5;
        uint256 private sellFeeLiquidity = 5;
        uint256 private sellOperationsFee = 2;
        ...
        uint256 public constant maxFee = 25; 


        uint256 private totalBuyFee = liquidityFee.add(treasuryFee).add(buyFeeRFV).add(referralFee).add(operationsFee).add(burnFee);
        uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeRFV).add(sellFeeTreasury).add(sellOperationsFee).add(sellBurnFee);
        ...
        uint256 public maxSellTransactionAmount = (INITIAL_FRAGMENTS_SUPPLY * 1) / 200;
        uint256 private maxSellTransactionDenom = 200; 
        ...
        uint256 private minSupplyForReferralReward = 1 * 10 **18 ; // 1 Token
        ...
        constructor(address _referralCA) ERC20Detailed("APY Bank", "APY", uint8(DECIMALS)) {
        ...
        function swapBack() internal swapping {
            ...
            uint256 swapOperationsFee = operationsFee.add(sellOperationsFee);
            uint256 realTotalFee =swapLiquidityFee.add(swapTreasuryFee).add(swapRFVFee).add(swapOperationsFee);
            ...
            uint256 amountToOperations = contractTokenBalance.mul(swapOperationsFee).div(realTotalFee);
            uint256 amountToTreasury = contractTokenBalance.sub(amountToLiquify).sub(amountToRFV).sub(amountToOperations).sub(1e18);
            ...
            if(amountToOperations > 0) {
                _swapTokensForBNB(amountToOperations, operationsFeeReceiver);
            }
            ...
            emit SwapBack(contractTokenBalance, amountToLiquify, amountToRFV, amountToOperations, amountToTreasury);
        }
        ...
        function takeBuyFee(address sender, address recipient, uint256 gonAmount) internal returns (uint256){
            ...
            uint256 tempReferralFee = referralFee; 

            if(isReferredBuyer) {
                referralFeeReceiver = referralCA.getReferrer(recipient);
                tempReferralFee = referralFeeWithReferral;
                }
            else{referralFeeReceiver = address(this);} //If not referred, fee will go to CA

            uint256 referralFeeAmount = gonAmount.mul(tempReferralFee).div(feeDenominator);
            uint256 feeAmount = gonAmount.mul((_realFee.sub(referralFee).sub(burnFee))).div(feeDenominator);
            uint256 burnFeeAmount = gonAmount.mul(burnFee).div(feeDenominator);
            uint256 totalFeeAmount = feeAmount.add(referralFeeAmount).add(burnFeeAmount);
            ...
            //Only emit event if tokens are transferred to DEAD
            if(burnFeeAmount > 0) {
                _gonBalances[DEAD] = _gonBalances[DEAD].add(burnFeeAmount);
                emit Transfer(sender, DEAD, burnFeeAmount.div(_gonsPerFragment));
            }
        ...
        function takeSellFee(address sender, uint256 gonAmount) internal returns (uint256){
            ...
            uint256 feeAmount = gonAmount.mul((_realFee).sub(sellBurnFee)).div(feeDenominator);
            uint256 burnFeeAmount = gonAmount.mul(sellBurnFee).div(feeDenominator);
        ...
        function takeTransferFee(address sender, uint256 gonAmount) internal returns (uint256){	
            ...
            //Only emit event if fee is active
            if (feeAmount > 0) {
                _gonBalances[address(this)] = _gonBalances[address(this)].add(feeAmount);	
                emit Transfer(sender, address(this), feeAmount.div(_gonsPerFragment)); 
            }
        ...
        function setFeeReceivers(address _liquidityReceiver, address _treasuryReceiver, address _riskFreeValueReceiver, address _operationsFeeReceiver) external onlyOwner {
            require(_liquidityReceiver != address(0) && _treasuryReceiver != address(0) && _riskFreeValueReceiver != address(0) && _operationsFeeReceiver != address(0), "Zero Address Validation");
            liquidityReceiver = _liquidityReceiver;
            treasuryReceiver = _treasuryReceiver;
            riskFreeValueReceiver = _riskFreeValueReceiver;
            operationsFeeReceiver = _operationsFeeReceiver;
            emit SetFeeReceivers(_liquidityReceiver, _treasuryReceiver, _riskFreeValueReceiver, _operationsFeeReceiver);
        }
        ...
        function setBuyFees(uint256 _liquidityFee, uint256 _treasuryFee, uint256 _buyFeeRFV, uint256 _referralFee, uint256 _burnFee, uint256 _operationsFee) external onlyOwner {
            ...
            totalBuyFee = liquidityFee.add(treasuryFee).add(buyFeeRFV).add(referralFee).add(burnFee).add(operationsFee);
            require(totalBuyFee < maxFee, "Fee can't be that high");

            emit BuyFeesChanged(_liquidityFee, _treasuryFee, _buyFeeRFV, _referralFee, _burnFee, _operationsFee);

        }

        function setSellFees(uint256 _liquidityFee, uint256 _treasuryFee, uint256 _sellFeeRFV, uint256 _burnFee, uint256 _operationsFee) external onlyOwner {
            sellFeeLiquidity = _liquidityFee;
            sellFeeTreasury = _treasuryFee;
            sellFeeRFV = _sellFeeRFV;
            sellBurnFee = _burnFee;
            sellOperationsFee = _operationsFee;
            totalSellFee = sellFeeLiquidity.add(sellFeeTreasury).add(sellFeeRFV).add(sellBurnFee).add(sellOperationsFee);
            require(totalSellFee < maxFee, "Fee can't be that high");

            emit SellFeesChanged(_liquidityFee, _treasuryFee, _sellFeeRFV, _burnFee, _operationsFee);
        }

        function setReferralFeeWithReferral (uint256 _fee) external onlyOwner {
            require(_fee <= 10, "Fee can't be that high");
            referralFeeWithReferral = _fee; 

            emit ChangedReferralFeeWithReferral(_fee);
        }
        ...
        function setMaxSellTransactionDenom(uint256 _maxSellDenom) external onlyOwner {
            require(_maxSellDenom > 0, "Can't be zero");
            maxSellTransactionDenom = _maxSellDenom;
            emit ChangedMaxSellTransactionDenom(_maxSellDenom);
        }
        ...
        function coreRebase(int256 supplyDelta) private returns (uint256) {
            ...
            _maxWalletSize = _totalSupply.div(maxWalletDenom);
            maxSellTransactionAmount = _totalSupply.div(maxSellTransactionDenom);
