...
/*
 https://ferretbsc.com
 TG: https://t.me/ferretbscofficial
*/
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  function updateDividendToken(address _DividendsToken) onlyOwner public {
  ...
  function withdrawDividend() public onlyOwner virtual override {
  ...
  function dividendOf(address _owner) public onlyOwner view override returns(uint256) {
    return withdrawableDividendOf(_owner);
  }

  function withdrawableDividendOf(address _owner) public onlyOwner view override returns(uint256) {
    return accumulativeDividendOf(_owner).sub(withdrawnDividends[_owner]);
  }

  function withdrawnDividendOf(address _owner) public onlyOwner view override returns(uint256) {
    return withdrawnDividends[_owner];
  }

  function accumulativeDividendOf(address _owner) public onlyOwner view override returns(uint256) {
...
contract Ferret is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   FerretDividendTracker public dividendTracker;
    ...
    uint256 private constant DefaultTime = 6 weeks;
    ...
    constructor() ERC20 ("Ferret", "Ferret") {
		...
		contractTokenBalanceAmount = 2500000 * (10**6) * 10**18;
        
        swapTokensAtAmount = 2500000 * (10**6) * (10**18);
        _maxWallet = 20000000 * (10**6) * (10**18);
        _maxBuy = 20000000 * (10**6) * (10**18);
        _maxSell = 20000000 * (10**6) * (10**18);
    	...
    	dividendTracker = new FerretDividendTracker();
		...
		marketingWallet = payable(0xD930D6d51b941647D7506D85fB8f890c47dC1B7b);
        ...
        */
        _mint(owner(), 1000000000 * (10**6) * (10**18));
    }
	...
	function updateSwapAmount(uint256 amount) public onlyOwner {
	    contractTokenBalanceAmount = amount * (10**6) * (10**18);
	    swapTokensAtAmount = amount * (10**6) * (10**18);
	}

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Ferret: The dividend tracker already has that address");

        FerretDividendTracker newDividendTracker = FerretDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Ferret: The new dividend tracker must be owned by the Ferret token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "Ferret: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Ferret: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Ferret: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "Ferret: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Ferret: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Ferret: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Ferret: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Ferret: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Ferret: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != liquidityWallet && to != liquidityWallet && from != marketingWallet && to != marketingWallet && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
		    ...
		    uint256 swapTokens;
			uint256 swapAmount = _sellMarketingFee.add(_sellRewardsFee).add(_sellLiquidityFee);
            ...
            transferToWallet(payable(marketingWallet), address(this).balance.mul(_sellMarketingFee).div(swapAmount));
            }
    ...
    function beforePresale() external onlyOwner() {
        ...
        _previousMaxWallet = _maxWallet;
        _previousMaxBuy = _maxBuy;
        _previousMaxSell = _maxSell;

        updateMaxWallet(1000000000 * (10**6) * (10**18));
        updateMaxBuySell((1000000000 * (10**6) * (10**18)), (1000000000 * (10**6) * (10**18)));
    }
    
	function afterPresale() external onlyOwner() {
        updateBuyFees(5,5,2);
        updateSellFees(8,5,2);

        updateMaxWallet(_previousMaxWallet);
        updateMaxBuySell(_previousMaxBuy, _previousMaxSell);
...
contract FerretDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Ferret_Dividend_Tracker", "Ferret_Dividend_Tracker") {
    	claimWait = 120;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "Ferret_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Ferret_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Ferret contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "Ferret_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Ferret_Dividend_Tracker: Cannot update claimWait to same value");
