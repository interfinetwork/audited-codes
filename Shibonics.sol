...
contract Shibonics is ERC20, Ownable {
    ...
    ShibonicsDividendTracker public dividendTracker;
    ...
    uint256 public ShibaInuRewardsSellFee = 4;
    ...
    uint256 public ShibaInuRewardsBuyFee = 4;
    uint256 public LiquidityBuyFee = 1;
    uint256 public MarketingBuyFee = 4;
    uint256 public ShibBurnBuyFee = 2;
    ...
    address payable public _marketingWalletAddress = payable(0xe9709414afFF5CB1383013AF861FBDa92d63E4A9);
    ...
    constructor() public ERC20("Shibonics", "SNis") {

        dividendTracker = new ShibonicsDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Shibonics: The dividend tracker already has that address");

        ShibonicsDividendTracker newDividendTracker = ShibonicsDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Shibonics: The new dividend tracker must be owned by the Shibonics token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Shibonics: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Shibonics: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Shibonics: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Shibonics: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Shibonics: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Shibonics: Cannot update gasForProcessing to same value");
...
contract ShibonicsDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("Shibonics_Dividen_Tracker", "Shibonics_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "Shibonics_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "Shibonics_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Shibonics contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Shibonics_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Shibonics_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  function distributeShibaInuDividends(uint256 amount) public onlyOwner{
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      bool success = IERC20(ShibaInu).transfer(user, _withdrawableDividend);
