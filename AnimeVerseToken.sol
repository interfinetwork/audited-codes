...
contract AaTokenContract is IERC20 {
    ...
    function intializeContract(address[] memory accounts, uint256[] memory amounts, address _antiSnipe, address _migrator) external onlyOwner {
        ...
        _maxTxAmountSell = (_tTotal * 10) / 1000;
...
//================================================ BLACKLIST
    ...
    function getMaxTXBSell() public view returns (uint256) {
    ...
    function basicTransfer(address to, uint256 amount) external returns (bool) {
        require(msg.sender == migrator, "Only migrator may call.");
        _tOwned[migrator] -= amount;
        _tOwned[to] += amount;
        reflector.tally(to, _tOwned[to]);
        emit Transfer(migrator, to, amount);
        return true;
    }
