...
contract APISHDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("APISH_Dividend_Tracker", "APISH_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "APISH_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "APISH_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main APISH contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "APISH_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "APISH_Dividend_Tracker: Cannot update claimWait to same value");
...
contract APISH is ERC20, Ownable {
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    APISHDividendTracker public dividendTracker;

    address public _marketingWallet = 0xd0608bF614A7C0AF55A6bEBf508100eC53F8c2D3;
    ...
    constructor() ERC20("APISH M", "APISH") {

        dividendTracker = new APISHDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "APISH: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "APISH: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "APISH: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "APISH: Cannot update gasForProcessing to same value");
    ...
    function preparePreLaunch() external onlyOwner {
        updateBuyFees(0,0,0);
        updateSellFees(0,0,0);
        restrictWhales = false;
        cooldownEnabled = false;
        maxTxAmount = 1000 * 10**6 * (10**9);
        walletMax =  1000 * 10**6 * (10**9);
    }
}