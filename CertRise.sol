...
contract CERTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CERT_Dividend_Tracker", "CERT_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override pure {
        require(false, "CERT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override  pure {
        require(false, "CERT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CERT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 600 && newClaimWait <= 86400, "CERT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CERT_Dividend_Tracker: Cannot update claimWait to same value");
...
contract CertRise is ERC20, Ownable {
    using SafeMath for uint256;

    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    CERTDividendTracker public dividendTracker;

    address public _marketingWallet = 0xd76c288f4BbbaEE8ACcdDe28ECb47B85246De112;
    ...
    constructor() ERC20("CertRise", "CERT") {

        dividendTracker = new CERTDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CERT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CERT: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CERT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CERT: Cannot update gasForProcessing to same value");
