...
contract AngryFloki is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public constant DOGE = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43); //DOGECOIN
    uint256 public constant MAX_INT = 2**256 - 1;

    uint256 public swapTokensAtLow = 500000000 * (10**9);
    uint256 public swapTokensAtHigh = 5000000000 * (10**9);
    ...
    constructor() public ERC20("AngryFloki", "$AFLOKI") {
        ...
        _marketingAddress = 0x12aaadb62418C252F462dc201B287C9a097291a9;
        _charityAddress = 0xC844b93d0E79f3a432dC376004D3af86AF81A97E;
        _buybackAddress = 0x5449690e7133C0c9c750D7A164217E3E71BE50c6;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        bool canSwap = contractTokenBalance >= swapTokensAtHigh;
    ...
    function utilizeFee() internal {
        uint256 amount = _genSwapAmount();

        uint256 marketingTotal = _marketingFeeTotal;
        uint256 charityTotal = _charityFeeTotal;
        uint256 buybackTotal = _buybackFeeTotal;
        uint256 liquidityTotal = _liquidityFeeTotal;
        uint256 rewardTotal = _rewardFeeTotal;

        uint256 currentTotal = marketingTotal.add(charityTotal).add(buybackTotal).add(liquidityTotal).add(rewardTotal);

        uint256 tokenForMarketing = amount.mul(marketingTotal).div(currentTotal);
        uint256 tokenForCharity = amount.mul(charityTotal).div(currentTotal);
        uint256 tokenForBuyback = amount.mul(buybackTotal).div(currentTotal);
        uint256 tokenForLiqudity = amount.mul(liquidityTotal).div(currentTotal);
        uint256 tokenForReward = amount.mul(rewardTotal).div(currentTotal);

        uint256 tokenForBNB = amount.sub(tokenForLiqudity.div(2));
        uint256 currentBNB = swapTokensForBNB(tokenForBNB);

        {
            uint256 BNBForMarketing = currentBNB.mul(tokenForMarketing).div(tokenForBNB);
            payable(_marketingAddress).transfer(BNBForMarketing);
        }

        {
            uint256 BNBForCharity = currentBNB.mul(tokenForCharity).div(tokenForBNB);
            payable(_charityAddress).transfer(BNBForCharity);
        }

        {
            uint256 BNBForBuyback = currentBNB.mul(tokenForBuyback).div(tokenForBNB);
            payable(_buybackAddress).transfer(BNBForBuyback);
        }

        {
            uint256 BNBForReward = currentBNB.mul(tokenForReward).div(tokenForBNB);
            swapAndSendDividends(BNBForReward);
        }

        addToLiquidity(tokenForLiqudity.div(2), address(this).balance);

       _marketingFeeTotal = marketingTotal.sub(tokenForMarketing);
       _charityFeeTotal = charityTotal.sub(tokenForCharity);
       _buybackFeeTotal = buybackTotal.sub(tokenForBuyback);
       _liquidityFeeTotal = liquidityTotal.sub(tokenForLiqudity);
       _rewardFeeTotal = rewardTotal.sub(tokenForReward);
    }
    ...
    function addToLiquidity(uint256 tokens, uint256 bnb) private {
    ...
    function swapAndSendDividends(uint256 bnbAmount) private{
        address[] memory path = new address[](2);
    ...
    function _genSwapAmount() internal returns (uint256) {
        uint256 pr = uint256(keccak256(abi.encodePacked(tx.origin, block.timestamp, _nonce++)));
        return swapTokensAtLow.add(pr.mod(swapTokensAtHigh.sub(swapTokensAtLow)));
    }

    function setSwapTokensAtAmount(uint256 low, uint256 high) external onlyOwner {
        require(high > low, "AFLOKI: high must be more than low");
        swapTokensAtLow = low;
        swapTokensAtHigh = high;
      }
}
