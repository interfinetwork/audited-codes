...
contract CATVILLS is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public REWARD = address(this);

    IPinkAntiBot public pinkAntiBot;
    bool public antibotEnabled = true;

    string constant _name = "CATVILLSCOIN";
    string constant _symbol = "CATVILLS";
    ...
    // buy fees
    uint256 public buyDividendRewardsFee = 1;
    uint256 public buyMarketingFee = 3;
    uint256 public buyLiquidityFee = 3;
    uint256 public buyShillerFee = 1;
    ...
    uint256 public sellShillerFee = 1;
    uint256 public sellDevFee = 1;
    uint256 public sellBurnFee = 0;
    uint256 public sellTotalFees = 10;

    address public marketingFeeReceiver = 0x6681583Df295F611961FC63caa4D470f267aFdCE;
    address public shillerFeeReceiver = 0xD0273B2fAc04A7ecf310a21ba9AFDe25f7262C79;
    address public devFeeReceiver = 0x390483352AC823bE1EB8f5F99a250620898007F5;
    address public liquidFeeReceiver = 0x390483352AC823bE1EB8f5F99a250620898007F5;
    ...
    constructor(address pinkAntiBot_, address _ownerAddress) Ownable(_ownerAddress) {
        ...
        pinkAntiBot.setTokenOwner(_ownerAddress);
    ...
    function updateBuyFees(
        ...
        buyShillerFee = shiller;
        buyDevFee = dev;
        buyBurnFee = burn;
        buyTotalFees = reward.add(marketing).add(liquidity).add(shiller).add(dev).add(burn);
    ...
    function updateSellFees(
        ...
        sellShillerFee = shiller;
        sellDevFee = dev;
        sellBurnFee = burn;
        sellTotalFees = reward.add(marketing).add(liquidity).add(shiller).add(dev).add(burn);
    ...
    function swapBackInBnb() internal swapping {
        ...
        if (shillerFeeBnb > 0) {
            (bool shillerSuccess, ) = payable(shillerFeeReceiver).call{
                value: shillerFeeBnb,
                gas: 30000
            }("");
            // only to supress warning msg
            shillerSuccess = false;
        }
    ...
    function swapAndSendFees(uint256 tokensForFee) private {
        ...
        uint256 shillerToken = currentTokenBalance.mul(buyShillerFee).div(totalSwapFee);
        uint256 devToken = currentTokenBalance.mul(buyDevFee).div(totalSwapFee);
        ...
        if (shillerToken > 0) {
            _approve(address(this), shillerFeeReceiver, shillerToken);
            IERC20(SWAPTOKEN).transfer(shillerFeeReceiver, shillerToken);
            emit SendFeesInToken(shillerFeeReceiver, shillerToken);
        }
    ...
    function setFeeReceivers(
		...
		, address _shillerFeeReceiver
		, address _devFeeReceiver
	) external onlyOwner {
        marketingFeeReceiver = _marketingFeeReceiver;
        shillerFeeReceiver = _shillerFeeReceiver;
