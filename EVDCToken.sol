    ...
    ERC20PausableUpgradeable
{
    ...
    string private    _name = "EVDC";
    string private  _symbol = "EVDC";
    uint8 private _decimals = 8;
    
    uint256 public _BuytaxFee = 0; // % of transactions will be redistributed among holders
    uint256 public _BuyliquidityFee = 200; // % of transactions will be kept for liquidity
    uint256 public _BuyburnFee = 0; // % of transactions will be burned
    uint256 public _BuymarketingFee = 150; // % of transactions will be kept for marketing wallet
    uint256 public _SelltaxFee = 50; // % of transactions will be redistributed among holders
    uint256 public _SellliquidityFee = 700; // % of transactions will be kept for liquidity
    uint256 public _SellburnFee = 0; // % of transactions will be burned
    uint256 public _SellmarketingFee = 350; // % of transactions will be kept for marketing wallet
    uint256 public _transferTaxFee = 0;
    uint256 public _transferLiquidityFee = 100;
    uint256 public _transferburnFee = 0;
    uint256 public _transfermarketingFee = 250;
    ...
    address public marketingWallet = 0x69d96053Ec00Fa5413AAd33A19f9380CC7F75456;
    ...
    function _transfer(
     ...
     //   if(paused == true)
       //     require(from == owner(), "contract is paused");
        ...
        bool takeFee;
        if(isBuyTransaction(from,to)){buyFee(); takeFee = true;}
        else if (isSellTransaction(from,to))
        {
            if(_tOwned[from] >= 100000000 * (10 ** 8))
            {
              require(block.timestamp >= lastsellTime[from] + 1 days, " You can not sell again before 24 hours.");
        ...
        else{transferFee(); takeFee = true;}
        
        uint256 marketingShare = calculateMarketingFee(amount);
    ...
    function sendtoMarktingWallet(address sender, address recipient, uint256 amount) private returns(bool)
    {
    ...
    function _mint(address account, uint256 amount) internal override {
        ...
        require(
            getMaxTotalSupply() >= _totalSupply+amount,
