...
contract LazyCheems is Context, IBEP20, Ownable {
    ...
    uint256 private _maxTxn = 3699 * 10**9 * 10**8; // 3BNB
    uint256 private _maxWallet = 5 * 10**6 * 10**8; // 1% of 1bil
    ...
    string private _name = "LazyCheems";
    string private _symbol = "LZCHEEMS";
    ...
    function set_MaxWallet(uint256 maxWallet) external onlyOwner() {
        ...
        function set_liquidityFee(uint256 liqFees) external onlyOwner() {
        _liquidityFee = liqFees;
    }
...
function addToBlackList(address[] calldata addresses) external onlyOwner {
    ...
    function removeFromBlackList(address account) external onlyOwner {
     ...
     function _transfer(
        ...
        require(!_isSniper[sender] && !_isSniper[recipient], "This address is blacklisted");
        ...
        if (
            ...
            recipient != pcsV2Pair
                        ...
                        if  (launchedAt>0 && (!_isExcludedFromFee[recipient] && !authorizations[sender])){
                    require(amount+ balanceOf(recipient)<= _maxWallet,
