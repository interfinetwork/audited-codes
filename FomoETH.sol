...
contract FomoETHDividendTracker is DividendPayingToken, Ownable {
    ...
    event UpdateMinTokenBalanceForDividends(uint256 newTokenBalance, uint256 oldTokenBalance);
    ...
    function updateMinimumTokenBalanceForDividends(uint256 newTokenBalance) external onlyOwner {
        emit UpdateMinTokenBalanceForDividends(newTokenBalance, minimumTokenBalanceForDividends);
...
contract FomoETH is ERC20, Ownable {
    ...
    constructor() ERC20("FomoETH", "FomoETH") {
        ...
        updateUniswapV2Router(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function swapAndDistributeETH(uint256 tokens) private {
        ...
        bool success = IERC20(Ethereum).transfer(marketingWallet, forMarketing);
        ...
        success = IERC20(Ethereum).transfer(address(dividendTracker), forDividends);

        if (success) {
            dividendTracker.distributeDividends(forDividends);
