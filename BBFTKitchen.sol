...
interface IBKITAgent {
    function addBKITPoolToken(address _token) external;
    function delBKITPoolToken(address _token) external;
    ...
    function setsendBUSDThreshold(uint256 _amount) external;
...
// BKIT AGENT THAT WILL PERFORM ALL THE BUYING AND SELLING
contract BKITAgent is IBKITAgent {
    ...
    uint256 sendBUSDthreshold = 1000 * (10 ** 18);
    uint256 convertBNBtoBUSDamount = 0;
    uint256 currentIndex;

    address[] public bkitPool;
    mapping (address => uint256) public bkitBuy;
    mapping (address => uint256) public bkitSell;
    ...
    function addBKITPoolToken(address _token) external override onlyToken {
        bkitPool.push(_token);
    }

    // Remove token from the BKIT pool
    function delBKITPoolToken(address _token) external override onlyToken {
        for (uint256 i = 0; i < bkitPool.length; i++) {
            if (bkitPool[i] == _token) {

                bkitPool[i] = bkitPool[bkitPool.length - 1];
                bkitPool.pop();
    ...
    // Show BKITpool token
    function showBKITPoolToken(uint256 _position) external view onlyToken returns (address) {
        return bkitPool[_position];
    }
    function showBKITBuy(address _token) external view onlyToken returns (uint256) {
        return bkitBuy[_token];
    }
    function showBKITSell(address _token) external view onlyToken returns (uint256) {
        return bkitSell[_token];
    }

    // TOKEN OPERATIONS
    // Accept buy request from BKIT
    function processBuyRequest(address _token, uint256 _buyamount) external override onlyToken {
        bkitBuy[_token] = bkitBuy[_token].add(_buyamount);
        emit agentError(384, _token, _buyamount);
    }

    // Accept sell requets from BKIT
    function processSellRequest(address _token, uint256 _sellpercentage) external override onlyToken {
        ...
        bkitSell[_token] = bkitSell[_token].add(tokenSellAmount);
    ...
    function processAgentActions(uint256 gas) external override onlyToken {
        uint256 bkitPoolCount = bkitPool.length;

        if(bkitPoolCount == 0) { return; }
        ...
        while(gasUsed < gas && iterations < bkitPoolCount) {
            if(currentIndex >= bkitPoolCount){
                currentIndex = 0;
            }

            emit agentError(410, bkitPool[currentIndex], currentIndex);
            if(bkitSell[bkitPool[currentIndex]] > 0) {
                doSellToken(bkitPool[currentIndex]);
            } else {
                doBuyToken(bkitPool[currentIndex]);
            }
    ...
    function doBuyToken(address _token) public onlyToken {
        uint256 tokenBuyAmount = bkitBuy[_token];
    ...
    function doSellToken(address _token) public onlyToken {
        ...
        bkitSell[_token] = bkitSell[_token].sub(tokensToSell);
    }
    ...
    function doSendBUSDtoBBFT() external override onlyToken {
        ...
        require(busdBalance > sendBUSDthreshold, "Not enough BUSD to send");
    ...
    // Set the BUSD threshold to hit before sending BUSD to BBFT
    function setsendBUSDThreshold(uint256 _amount) external override onlyToken {
        sendBUSDthreshold = _amount;
    }

    // ADMIN FUNCTIONS
    // Clear unsuable BNB from agent wallet
    function clearStuckBKITBNB(address _wallet, uint256 _amount) external onlyToken {
...
// MAIN BBFT CONTRACT
contract BBFTKitchen is BKITAuth {
    using SafeMath for uint256;
    ...
    uint256 public buyThreshold = 4000 * (10 ** 18);
    uint256 public minBuyAmount = 150 * (10 ** 18);
    uint256 public sellPercentage = 20;
    ...
    BKITAgent kitchenagent;
    uint256 kitchenagentGas = 600000;
    ...
    function addPoolToken(address _token, uint256 _tokenWeight) external authorizedFor(Permission.AdjustContractVariables) {
        ...
        kitchenagent.addBKITPoolToken(_token);
    }
    ...
    function delPoolToken(address _token) external authorizedFor(Permission.AdjustContractVariables) {
        for (uint256 i = 0; i < tokenPool.length; i++) {
            if (tokenPool[i] == _token) {
                ...
                kitchenagent.delBKITPoolToken(_token);
    ...
    // Set the minimum number of BNB required to perform BKIT buys
    ...
    function requestTokenBuys() internal {
        if(checkBuyState()) { performTokenBuyRequests(); }

        kitchenagent.processAgentActions(kitchenagentGas);
    }
    ...
    function performTokenBuyRequests() internal swapping {
        ...
        emit bkitError(649, address(this), curBNBBuyAmount);
        for (uint256 i = 0; i < tokenPool.length; i++) {
            if(isBuyToken[tokenPool[i]]) {
                curTokenBuyAmount = curBNBBuyAmount.mul(tokenWeight[tokenPool[i]]).div(totalWeight);
                emit bkitError(653, tokenPool[i], curTokenBuyAmount);
                kitchenagent.processBuyRequest(tokenPool[i], curTokenBuyAmount);
            }
        }

        (bool sentBNB, ) = payable(address(kitchenagent)).call{value: curBNBBuyAmount}("");
        ...
        if(!inSwap) {
            for (uint256 i = 0; i < tokenPool.length; i++) {
                if(isSellToken[tokenPool[i]]) {
                    emit bkitError(679, tokenPool[i], sellPercentage);
                    kitchenagent.processSellRequest(tokenPool[i], sellPercentage);
                }
            }
        }
    }
    ...
    function manualConvertBNBtoBUSD(uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        if(address(this).balance > _amount) {
            kitchenagent.doManualBUSD(_amount);
            emit bkitError(691, address(this), _amount);
            (bool sentBNB, ) = payable(address(kitchenagent)).call{value: _amount}("");
            require(sentBNB, "BNB not sent");
        }
    }
    ...
    function clearStuckBKITBNB(address _wallet, uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        kitchenagent.clearStuckBKITBNB(_wallet, _amount);
    }
    function setsendBUSDThreshold(uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        kitchenagent.setsendBUSDThreshold(_amount);
    }
    function doBurnBBFT() external authorizedFor(Permission.AdjustContractVariables) {
        kitchenagent.doBurnBBFT();
    }

    // DEBUGGING FUNCTIONS
    function manualRequestTokenBuys() external authorizedFor(Permission.AdjustContractVariables) {
        emit bkitError(724, address(this), 0);
        requestTokenBuys();
    }
    function showBKITPoolToken(uint256 _position) external view authorizedFor(Permission.AdjustContractVariables) returns (address) {
        return kitchenagent.showBKITPoolToken(_position);
    }
    function showBKITBuy(address _token) external view authorizedFor(Permission.AdjustContractVariables) returns (uint256) {
        return kitchenagent.showBKITBuy(_token);
    }
    function showBKITSell(address _token) external view authorizedFor(Permission.AdjustContractVariables) returns (uint256) {
        return kitchenagent.showBKITSell(_token);
    }
    event bkitError(uint256 indexed _eventid, address indexed _eventaddr, uint256 indexed _eventvalue);
}