...
contract APO is Context, IERC20, Ownable {
    ...
    address private _walletAddress = 0x20598CFCa6C736f4313a0b9FcA87c1D4f49B4Cc5;
   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 113000  * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "APOLLO 13";
    ...
    uint256 public _maxTxAmount = 113000 * 10**18;
    uint256 private numTokensSellToAddToLiquidity = 1130 * 10**18;
    ...
    function includeInReward(address account) external onlyOwner() {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 twallet) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, twallet);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee = calculateTaxFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 twallet = calculatewalletFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(twallet);
    ...
    function _takewallet(uint256 twallet) private {
        uint256 currentRate =  _getRate();
        uint256 rwallet = twallet.mul(currentRate);
        _rOwned[_walletAddress] = _rOwned[_walletAddress].add(rwallet);
        if(_isExcluded[_walletAddress])
            _tOwned[_walletAddress] = _tOwned[_walletAddress].add(twallet);
    }
