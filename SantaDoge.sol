...
contract SantaDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public operationsWallet1; // operations divided for separate purposes (Marketing, team, etc.)
    address public operationsWallet2; // operations divided for separate purposes (Marketing, team, etc.)
    ...
    uint256 public gameSellFee;
    uint256 public jackpotSellFee;
    
    uint256 public totalBuyFees;
    uint256 public rewardsBuyFee;
    uint256 public operationsBuyFee;
    uint256 public liquidityBuyFee;
    uint256 public gameBuyFee;
    uint256 public jackpotBuyFee;
    ...
    constructor() ERC20("Santa Doge", "SantaDOGE") {
        address newOwner = 0x229557D6f82467840982eF27ceFD001DE37F824F;
    
        uint256 totalSupply = 100 * 1e9 * 1e18;
        
        maxTransactionAmount = totalSupply * 1 / 100; // 1% maxTransaction 
        ...
        gameSellFee = 2;
        jackpotSellFee = 1;
        totalSellFees = rewardsSellFee + operationsSellFee + liquiditySellFee + gameSellFee + jackpotSellFee;
        ...
        gameBuyFee = 2;
        jackpotBuyFee = 1;
        totalBuyFees = rewardsBuyFee + operationsBuyFee + liquidityBuyFee + gameBuyFee + jackpotBuyFee;
        
    	dividendTracker = new DividendTracker();
    	
    	operationsWallet1 = address(0x55212A6f74BA859B295be0D8943E5bcac677CD62);
    	operationsWallet2 = address(0x615d900eA911f73b26fD22F44A464515495E5D5D);
        gameWallet = address(0x618a7Af724455e49D1eA913d49b7A1CcDA6d030E);
        jackpotWallet = address(0x80fB6060f508C5BF0D338CdfB06a143B8ad211D4);
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _gameFee, uint256 _jackpotFee) external onlyOwner {
        operationsBuyFee = _operationsFee;
        rewardsBuyFee = _rewardsFee;
        liquidityBuyFee = _liquidityFee;
        gameBuyFee = _gameFee;
        jackpotBuyFee = _jackpotFee;
        totalBuyFees = operationsBuyFee + rewardsBuyFee + liquidityBuyFee + gameBuyFee + jackpotBuyFee;
        require(totalBuyFees <= 20, "Must keep fees at 20% or less");
    }
    
    function updateSellFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _gameFee, uint256 _jackpotFee) external onlyOwner {
        operationsSellFee = _operationsFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
        gameSellFee = _gameFee;
        jackpotSellFee = _jackpotFee;
        totalSellFees = operationsSellFee + rewardsSellFee + liquiditySellFee + gameSellFee + jackpotSellFee;
    ...
    function updateOperationsWallet1(address newOperationsWallet) external onlyOwner {
        excludeFromFees(newOperationsWallet, true);
        emit operationsWalletUpdated(newOperationsWallet, operationsWallet1);
        operationsWallet1 = newOperationsWallet;
    }

    function updateOperationsWallet2(address newOperationsWallet) external onlyOwner {
        excludeFromFees(newOperationsWallet, true);
        emit operationsWalletUpdated(newOperationsWallet, operationsWallet2);
        operationsWallet2 = newOperationsWallet;
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && totalSellFees > 0){
                ...
                tokensForJackpot += fees * jackpotSellFee / totalSellFees;
                tokensForGame += fees * gameSellFee / totalSellFees;
            ...
            // on buy
                ...
                tokensForJackpot += fees * jackpotBuyFee / totalBuyFees;
                tokensForGame += fees * gameBuyFee / totalBuyFees;
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForOperations + tokensForRewards + tokensForGame + tokensForJackpot;
        ...
        uint256 ethForJackpot = ethBalance.mul(tokensForJackpot).div(totalTokensToSwap);
        uint256 ethForRewards = ethBalance.mul(tokensForRewards).div(totalTokensToSwap);
        
        uint256 ethForLiquidity = ethBalance - ethForOperations - ethForGame - ethForRewards - ethForJackpot;
        ...
        (bool success,) = address(operationsWallet1).call{value: ethForOperations/2}("");
        (success,) = address(operationsWallet2).call{value: ethForOperations/2}("");
        ...
        // keep leftover ETH for buyback only if there is a buyback fee, if not, send the remaining ETH to the operations wallet if it accumulates
        if(ethForRewards > 0){
