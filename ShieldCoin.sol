...
contract ShieldCoin is IBEP20, SHCAuth {
    using SafeMath for uint256;
    ...
    string constant _name = "ShieldCoin";
    string constant _symbol = "SHC";
    ...
    uint256 public operationFee = 50;
    uint256 public VaultFee = 50;
    uint256 public burnFee = 100;
    uint256 totalFee = 900;
    uint256 public feeDenominator = 10000;

    address public autoLiquidityReceiver;
    address public operationFeeReceiver =0xd1a5444F99BE6C3EefBc6998c5e7F0F069025d98;
    address public VaultFeeReceiver =0xd1a5444F99BE6C3EefBc6998c5e7F0F069025d98;
    ...
    constructor () SHCAuth(msg.sender) {
    ...
    function swapBack() internal swapping {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 amountBNBMarketing = amountBNB.mul(operationFee).div(totalBNBFee);
            uint256 amountBNBBuyback = amountBNB.mul(VaultFee).div(totalBNBFee);

            try distributor.deposit{value: amountBNBReflection}() {} catch {}
            (bool marketSuccess, ) = payable(operationFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
            (bool buybackSuccess, ) = payable(VaultFeeReceiver).call{value: amountBNBBuyback, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _operationFee, uint256 _VaultFee, uint256 _burnFee, uint256 _feeDenominator) external authorizedFor(Permission.AdjustContractVariables) {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        operationFee = _operationFee;
        VaultFee = _VaultFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_operationFee).add(_VaultFee).add(_burnFee);
        feeDenominator = _feeDenominator;
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _operationFeeReceiver, address _VaultFeeReceiver) external authorizedFor(Permission.AdjustContractVariables) {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        operationFeeReceiver = _operationFeeReceiver;
