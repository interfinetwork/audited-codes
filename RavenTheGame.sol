...
//
//
// Earn to play mobile-web based game
//
//
...
contract Raventhegame is Context, IERC20, Ownable {
    ...
    modifier antiBot(address account){
        require(t_Enabled || allowedTransfer[account], "Trading not enabled yet");
        _;
    }
    ...
    address public marketingWallet = 0x892f647A5f9103E3240ba552aDCc793871E2f18f;
    address public devWallet = 0x073bbc306ACd3BB6196C860C9336F3468c16a9df;

    string private constant _name = "RaventheGame";
    string private constant _symbol = "RTG";
    ...
    function bl_check(address account) public view returns (bool) {
    ...
    function setTStatus(bool state) external onlyOwner{
        require(state != false, "You can not stop trading after it is started");
        t_Enabled = state;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _dev) public onlyOwner {
       require((_rfi + _marketing + _liquidity + _dev) <= 12, "Tax is too much");
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!_isBlacklisted[from] && !_isBlacklisted[to], "Bot protection");
        }
        
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            require(t_Enabled, "Trading is not active yet");
        }
        
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= genesis_block + 170) {
            require(to != pair, "Sells not allowed for first 170 blocks for blacklisting bot buyers");
        }
        ...
        if(to == pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
        ...
        if(canSwap && from != pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to] && !swapping){
    ...
    function updateIsBl(address account, bool state) external onlyOwner{
        require(account != pair , "Pair address can not be blacklisted");
        require(account != address(router) , "Router address can not be blacklisted");
        _isBlacklisted[account] = state;
    }
    
    function bulkIsBl(address[] memory accounts, bool state) external onlyOwner{
        
        for(uint256 i =0; i < accounts.length; i++){
            require(accounts[i] != pair , "Pair address can not be blacklisted");
            require(accounts[i] != address(router) , "Router address can not be blacklisted");
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        require(maxBuy >= ((_tTotal/(10**decimals()))/2000) , "Too less");
        require(maxSell >= ((_tTotal/(10**decimals()))/2000) , "Too less");
