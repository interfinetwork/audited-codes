...
contract LockedStaking is Ownable {
    ...
    function setupPool(IERC20 _stakeToken, IERC20 _rewardToken, uint256 _rewardPerBlock, uint256 _lockupDuration, uint256 _stakeMinimum) external onlyOwner {
        require(poolInfo.length == 0, "Pool is already setup");
    ...
    function startStaking(uint256 startBlock) external onlyOwner {
        require(poolInfo.length > 0, "Pool is not setup");
    ...
    function setLockupDuration(uint256 _lockupDuration) external onlyOwner {
        require(poolInfo.length > 0, "Pool is not setup");
        poolInfo[0].lockupDuration = _lockupDuration;
    }

    function setStakeMinimum(uint256 _stakeMinimum) external onlyOwner {
        require(poolInfo.length > 0, "Pool is not setup");
        poolInfo[0].stakeMinimum = _stakeMinimum;
    }

    function pendingRewards(address _user) external view returns (uint256) {
        require(poolInfo.length > 0, "Pool is not setup");
        require(poolInfo[0].lastRewardBlock > 0 && block.number >= poolInfo[0].lastRewardBlock, 'Staking not yet started');
        ...
        if (block.number > pool.lastRewardBlock && depositedAmount != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(reward.mul(1e12).div(depositedAmount));
        }
    ...
    function updatePool() internal {
        ...
        pool.accRewardPerShare = pool.accRewardPerShare.add(reward.mul(1e12).div(depositedAmount));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 amount) external {
        ...
        require(amount >= pool.stakeMinimum, "Amount is less than the stake minimum");

        updatePool();

        bool usersFirstDeposit = user.amount == 0;
        ...
        if (amount > 0) {
            stakeToken.safeTransferFrom(address(msg.sender), address(this), amount);
            user.amount = user.amount.add(amount);
            pool.depositedAmount = pool.depositedAmount.add(amount);
        }
        user.rewardDebt = user.amount.mul(pool.accRewardPerShare).div(1e12);

        if(usersFirstDeposit) {
    ...
    function withdraw(uint256 amount) external {
        ...
        UserInfo storage user = userInfo[0][msg.sender];
        require(poolInfo.length > 0, "Pool is not setup");
        require(block.timestamp > user.lockupStart + pool.lockupDuration, "Stake is locked");
        require(user.amount >= amount, "Withdrawing more than your stake");
        updatePool();
        uint256 pending = user.amount.mul(pool.accRewardPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            user.pendingRewards = user.pendingRewards.add(pending);
    ...
    function safeRewardTransfer(address to, uint256 amount) internal returns (uint256) {
        ...
        pool.rewardReserve = pool.rewardReserve.sub(rewardAmount);
        return rewardAmount;
    }
    ...
    function depositRewardReserve(uint256 amount) external onlyOwner {
        ...
        PoolInfo storage pool = poolInfo[0];
        pool.rewardReserve = pool.rewardReserve.add(amount);
    }
    ...
    function withdrawRewardReserve(uint256 amount) external onlyOwner {
        ...
        pool.rewardReserve = pool.rewardReserve.sub(amount);
        rewardToken.safeTransfer(address(owner()), amount);
    }
}