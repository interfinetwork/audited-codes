...
The Token With Rewards ETH (Etherium)

Telegram: https://t.me/hodlETHOfficial
Website: https://HodlETH.one
*/
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 HETH = IBEP20(0x2170Ed0880ac9A755fd29B2688956BD959F933F8);
    ...
    uint256 public minPeriod = 4 hours; // min 2 hours delay
    uint256 public minDistribution = 2/10000 * (10 ** 18); // 1 usd worth of  ETH Rewards minimum Auto Send
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = HETH.balanceOf(address(this));
        ...
        uint256 amount = HETH.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            HETH.transfer(shareholder, amount);
...
contract HodlETH is IBEP20, HETHAuth {
    using SafeMath for uint256;

    address HETH = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "Hodl ETH";
    string constant _symbol = "HETH";
    ...
    uint256 VaultFee = 200;
    uint256 reflectionFee = 600;
    uint256 marketingFee = 400;
    uint256 totalBuyFee = liquidityFee + VaultFee + reflectionFee + marketingFee;
    ...
    constructor () HETHAuth(msg.sender) {
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
           ...
           try distributor.deposit{value: amountBNBReflection}() {} catch {}
            (bool success, /* bytes memory data */) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
             (success, /* bytes memory data */) = payable(vaultFeeReceiver).call{value: amountBNBvault, gas: 30000}("");
    ...
    function setTxLimit(uint256 amount) external authorizedFor(Permission.AdjustContractVariables) {
        _maxTxAmount = _totalSupply.div(1000).mul(amount);
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _VaultFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator, uint256 _totalSellFee) external authorizedFor(Permission.AdjustContractVariables) {
        liquidityFee = _liquidityFee;
        VaultFee = _VaultFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        totalBuyFee = _liquidityFee.add(_VaultFee).add(_reflectionFee).add(_marketingFee);
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver , address _vaultFeeReceiver) external authorizedFor(Permission.AdjustContractVariables) {
    ...
    function setLaunchedAt(uint256 launched_) external authorizedFor(Permission.AdjustContractVariables) {
     ...
     function clearStuckBalance(uint256 amountPercentage) external authorizedFor(Permission.Authorize) {
    ...
    function clearStuckBalance_sender(uint256 amountPercentage) external authorizedFor(Permission.Authorize) {
        uint256 amountBNB = address(this).balance;
