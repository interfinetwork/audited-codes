...
contract Cheetah is ERC20Detailed, Ownable {
    ...
    function checkRebase(uint ratioBoost) internal view returns(uint256) {
        ...
        console.log("nft boost: %s applicato",nftRew[ratioBoost]);
        ...
        console.log("actual rebase ratio: %s - boosted ratio: %s - bonus ratio: %s",rebaseRate,rebaseBoostedRate,bonusRate);
        for (uint256 i = 0; i < times; i++) {
            _tsupply = _tsupply
                .mul((10**RATE_DECIMALS).add(rebaseRate))
                .div(10**RATE_DECIMALS);
        ...
        console.log("normal supply: %s - boosted: %s",_tsupply,boostedSupply);
        //4697:1 = 0.02 : X
        //1 * 0.02 = 4697: X
        //X = (1*0.02) / 4697
    ...
    function calcRewardNft2(address who) public view returns(uint){
        ...
        console.log("diff supply: %s",calcSupply);
        if(nftBal>5){nftBal=5;}
        uint bonus = nftRew[nftBal] * calcSupply / 10000;
        
        uint256 newSupply = _totalSupply + bonus;
        console.log("bonus: %s - new supply: %s - attuale: %s",bonus,newSupply,_totalSupply);
