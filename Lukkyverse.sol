...
contract LukyLepereuchen is Context, IERC20, Ownable {
    ...
    string private constant _name = "Test Test";
    ...
    event BigJackpotWinner (address indexed user, uint256 time);
    event DailyJackpotWinner (address indexed user, uint256 time);
    event JackpotFeaturesSet( uint256 _jackpotCashout,
    ...
    event JackpotHardFeaturesSet(uint256 _jackpotHardBuyback,
   ...
   antiBotEnabled = _enable;
    ...
    function setJackpotFeatures(
        ...
        emit JackpotFeaturesSet( _jackpotCashout,
         _jackpotBuyerShare,
         _jackpotMinBuy,
         _miniJackpotWinners,
         _miniJackpotAmount);
    ...
    function setJackpotHardFeatures(
        ...
        emit JackpotHardFeaturesSet( _jackpotHardBuyback,
         _jackpotHardLimit);
    }
    ...
    function _transfer(
        ...
        if(block.timestamp - _lastDailyTimestamp >= 1 days){
            ...
            if(_pendingJackpotBalance >= bnbamount  && !inSwapAndLiquify){
        ...
        if(!isExcludedFromDailyWin[from]) tokenHoldersMap.set(from,balanceOf(from));
   ...
   function processDailyWinner(uint256 amount) private lockTheSwap {
    ...
    function awardJackpot() private lockTheSwap {
        ...
        _pendingJackpotBalance = _pendingJackpotBalance.sub(cashedOut);
        _jackpotTokens = _jackpotTokens.sub(tokensOut);
        emit BigJackpotWinner(_lastBuyer, block.timestamp);
