...
contract RPM is  IERC20, Ownable {
    ...
    string private _name = "RapidMoon";
    string private _symbol = "RPM";
    ...
    function ChangeP2EWallet(address newWallet) public onlyOwner{
        P2EWallet=newWallet;
    }
    address[] MarketingWallets;
    uint[] MarketingShares;
    function SetMarketingWallets(address[] memory wallets, uint[] memory shares) public{
        require(wallets.length==shares.length);
        require(wallets.length<=6);
        MarketingWallets=new address[](wallets.length);
        MarketingShares=new uint[](wallets.length);
        uint percent=0;
        for(uint i=0;i<wallets.length;i++){
            MarketingWallets[i]=wallets[i];
            MarketingShares[i]=shares[i];
            percent+=shares[i];
        }
        require(percent==100,"shares don't add up to 100");
    }
    ...
    constructor ()  {
        ...
        P2EWallet=msg.sender;
        MarketingWallets=new address[](1);
        MarketingWallets[0]=msg.sender;
        MarketingShares=new uint[](1);
        MarketingShares[0]=100;
    ...
    function _getValues(uint256 tAmount, uint taxes) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function _getTValues(uint256 tAmount, uint taxes) private view returns (uint256, uint256, uint256) {
    ...
    function calculateReflectionFee(uint256 _amount, uint taxes) private view returns (uint256) {
        return (_amount*_reflectionTax*taxes)/(TAX_DENOMINATOR*TAX_DENOMINATOR);
    }

    function calculateContractFee(uint256 _amount, uint taxes) private view returns (uint256) {
        return (_amount*(_liquidityTax+_marketingTax+_P2ETax)*taxes)/(TAX_DENOMINATOR*TAX_DENOMINATOR);
    }
...
////////////////////////////////////////////////////////////////////////////////
    ...
    uint _marketingTax=500;
    uint _liquidityTax=166;
    uint _P2ETax=167;
    uint _reflectionTax=167;
    ...
    function SetTaxes(uint LP_, uint marketing_, uint P2E_,uint reflection_, uint buy_, uint sell_, uint transfer_) public onlyOwner{
        require((LP_+marketing_+reflection_+P2E_)==TAX_DENOMINATOR);
        uint MaxTax=TAX_DENOMINATOR/5;//20%
        require(buy_<MaxTax&&sell_<MaxTax&&transfer_<MaxTax);
        _liquidityTax=LP_;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    function _swapContractToken() private lockTheSwap{
        ...
        uint totalTax=_liquidityTax+_marketingTax;
        ...
        uint256 swapToken=liqToken+tokenForBNB;
        if(swapToken==0) return;
        swapTokenForBNB(swapToken);
        //calculates the amount of BNB belonging to the LP-Pair and converts them to LP
        uint256 liqBNB = (address(this).balance*liqToken)/swapToken;
        if(liqBNB>0&&liqToken>0)
            addLiquidity(liqToken, liqBNB);
        uint TotalRemainingTax=_marketingTax+_P2ETax;
        uint MarketingAmount=(address(this).balance*_marketingTax/TotalRemainingTax)/4;

        bool sent;
        for(uint i=0;i<MarketingWallets.length;i++)
        {
            if(i>=MarketingShares.length)return;
            (sent,)=MarketingWallets[i].call{value:MarketingAmount*MarketingShares[i]/100}("");
        }
        (sent,)=P2EWallet.call{value:address(this).balance}("");
...
////////////////////////////////////////////////////////////////////////////////
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(takeFee){
            ...
            //Bot protection Tax
            uint BuyTaxDuration=2 minutes;
            if(block.timestamp<LaunchTimestamp+BuyTaxDuration){
                uint timeSinceLaunch=block.timestamp-LaunchTimestamp;
                taxes= TAX_DENOMINATOR-((TAX_DENOMINATOR-_buyTax)*timeSinceLaunch/BuyTaxDuration);
            }else taxes=_buyTax;

            } 
            else{//SELL OR TRANSFER
            if(recipient==_pancakePairAddress) taxes=_sellTax;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, uint taxes) private {
    ...
    function _transferToExcluded(address sender, address recipient, uint256 tAmount, uint taxes) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount,taxes);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount, uint taxes) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount,taxes);
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount, uint taxes) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount,taxes);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
