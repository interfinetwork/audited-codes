...
contract Archrealm is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 2_500_000 * 10**9;
    ...
    address public marketingWallet = 0x60c1A93f8FeA0846049334A3c208B761C368A085;
    address public operationWallet = 0xf35A13a996C9A30d64288815E3f157A325ADB3D9;
    address public gamerewardWallet = 0x4fD3472900E9502fAf2511373e53b6c5021b2fBF;

    string private constant _name = "Archrealm";
    ...
    Taxes public taxes =     Taxes(2, 4, 2, 1, 2);
    Taxes public sellTaxes = Taxes(2, 4, 2, 1, 2);
    ...
    constructor(address routerAddress) {
        ...
        _isExcludedFromFee[gamerewardWallet] = true;
        ...
        allowedTransfer[gamerewardWallet] = true;
    ...
    function setTradingStatus(
        ...
        swapEnabled = state;
        deadline = _deadline;
        launchtax = Taxes(rfi, marketing, liquidity, operation, gamereward);
    ...
    function setTaxes(
        ...
        taxes = Taxes(_rfi, _marketing, _liquidity, _operation, _gamereward);
        emit FeesChanged();
    }
    ...
    function setSellTaxes(
        ...
        sellTaxes = Taxes(_rfi, _marketing, _liquidity, _operation, _gamereward);
        emit FeesChanged();
    }
    ...
    function _takeGamereward(uint256 rGamereward, uint256 tGamereward) private {
        totFeesPaid.gamereward += tGamereward;

        if (_isExcluded[address(this)]) {
            _tOwned[address(this)] += tGamereward;
        }
        _rOwned[address(this)] += rGamereward;
    }
    ...
    function _getValues(
        ...
        (to_return.rOperation, to_return.rGamereward) = _getRValues2(
            to_return,
            takeFee,
            _getRate()
        );
    ...
    function _getTValues(
        ...
        s.tGamereward = (tAmount * temp.gamereward) / 100;
        s.tTransferAmount =
            tAmount -
            s.tRfi -
            s.tMarketing -
        ...
        )
    {
        ...
        uint256 rGamereward = s.tGamereward * currentRate;
        rTransferAmount =
            rAmount -
            rRfi -
            rMarketing -
        ...
        )
    {
        ...
        rGamereward = s.tGamereward * currentRate;
        return (rOperation, rGamereward);
    }
    ...
    function _tokenTransfer(
        ...
        if (s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(
                sender,
                address(this),
                s.tLiquidity + s.tMarketing + s.tOperation + s.tGamereward
            );
        }
        if (s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if (s.rGamereward > 0 || s.tGamereward > 0) _takeGamereward(s.rGamereward, s.tGamereward);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap {
        ...
        uint256 gamerewardAmt = unitBalance * 2 * temp.gamereward;
        if (gamerewardAmt > 0) {
            payable(gamerewardWallet).sendValue(gamerewardAmt);
        }
    }
