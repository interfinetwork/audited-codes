...
contract SCROHOLDINGSLLC is IBEP20, Auth {
    ...
    string constant _name = "SCRO Holdings LLC";
    string constant _symbol = "SCRO";
    uint8 constant _decimals = 4;

    uint256 _totalSupply = 12 * 10**8 * 10**_decimals;
    ...
    uint256 public magFee = 2;
    uint256 public devFee = 1;
    uint256 public utilityFee = 0;
    uint256 public totalFee = marketingFee + liquidityFee + magFee + utilityFee + devFee;
    uint256 public constant feeDenominator = 100;

    uint256 sellMultiplier = 133;
    uint256 buyMultiplier = 66;
    uint256 transferMultiplier = 133;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x114B3757bD2D637e8f2E0fC142853768E40cCBF8;
        magFeeReceiver = 0xeA6402c582D358fBfEB97adD349df36E9D3B50be;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBmag = amountBNB.mul(magFee).div(totalBNBFee);
        uint256 amountBNBDev = amountBNB.mul(devFee).div(totalBNBFee);

        payable(marketingFeeReceiver).transfer(amountBNBMarketing);
        payable(magFeeReceiver).transfer(amountBNBmag);
    ...
    function setFees(uint256 _liquidityFee,  uint256 _marketingFee, uint256 _magFee, uint256 _utilityFee, uint256 _devFee) external onlyOwner {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        magFee = _magFee;
        devFee = _devFee;
        utilityFee = _utilityFee;
        totalFee = _liquidityFee + _marketingFee + _magFee + _utilityFee + _devFee;
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _magFeeReceiver, address _utilityFeeReceiver, address _devFeeReceiver) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        magFeeReceiver = _magFeeReceiver;
