...
/**
                            ...
                            _____     _       
     /\                    |  __ \   | |      
    /  \   __ _ _   _  __ _| |__) |__| |_ ___ 
   / /\ \ / _` | | | |/ _` |  ___/ _ \ __/ __|
  / ____ \ (_| | |_| | (_| | |  |  __/ |_\__ \
 /_/    \_\__, |\__,_|\__,_|_|   \___|\__|___/
             | |                              
             |_|                              
    ...
    Tokenomics:
    10% Buy Tax in BNB
    2% Liquidity | 2% Charity | 5% Marketing | 1% Development

    14% Sell Tax in BNB
    2% Liquidity | 2% Charity | 6% Marketing | 2% Buyback | 2% Development
*/
...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public constant BUSD = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c); //WBNB
...
pragma solidity ^0.8.10;
contract AQUAPETS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public swapTokensAtAmount = 1 * 10**10 * (10**9); //0.01% of the supply
    
    // Anti Whale features
    uint256 public maxBuyAmount = 3 * 10**7 * 10**9; // 3% of the supply
    uint256 public maxSellAmount = 3 * 10**7 * 10**9; // 3% of the supply
    uint256 public maxWalletAmount = 3 * 10**7 * 10**9; // 3% of the supply
    ...
    address public _developmentWallet = payable(address(0xe9EF59106401722fe12fc4E57C6075092331D5dd)); //developmentWallet
    address public _marketingWallet = payable(address(0x5D27FF616079e175e38c7740BfC6B5f59Ac86Bce)); //marketingWallet
    address public _charityWallet = payable(address(0xC1c35495f0D60bb8ccb4E4eD768559b8fDb812D9)); //charityWallet
    address public _buybackWallet = payable(address(0x12C42E0A024Bd0146A96D8E837586A28E20e5421)); //buybackWallet
    ...
    constructor() ERC20("AquaPets", "AQP") {
        buyFee.rewardFee = 0;
        buyFee.devfee = 1;
        ...
        */
        _mint(owner(), 1 * 10**9 * (10**9)); // 1 Billion Tokens
    }
   ...
   function setDevelopmentWallet (address wallet) external onlyOwner {
    ...
    function setBuyFee(
        ...
        buyFee.charityFee = charity;
        buyFee.buybackFee = buyback;
        totalBuyFee = rewardfee + dev + liquidity + marketing + charity + buyback;
    }
    ...
    function setSellFee(
        ...
        sellFee.charityFee = charity;
        sellFee.buybackFee = buyback;
        totalSellFee = rewardfee + dev + liquidity + marketing + charity + buyback;
    }
   ...
   function setSwapTokensAtAmount(uint256 amount) external onlyOwner {
    ...
    function _transfer(
        ...
        require(isTradingEnabled || _isExcludedFromFees[from],"Trading not enabled yet");
        ...
        if (
            ...
            uint256 feeTokens = contractTokenBalance
                .mul(buyFee.devfee + sellFee.devfee + buyFee.marketingFee + sellFee.marketingFee + buyFee.charityFee + sellFee.charityFee + buyFee.buybackFee + sellFee.buybackFee)
                .div(totalFee);

            swapAndSendToWallets(feeTokens);
    ...
    function swapAndSendToWallets(uint256 tokens) private lockTheSwap {
        ...
        swapTokensForTokens(tokens, BUSD, address(this));
        uint256 newBalance = IERC20(BUSD).balanceOf(address(this)).sub(
            initialBalance);
        uint256 marketingShare = newBalance.mul(buyFee.marketingFee + sellFee.marketingFee).div(buyFee.devfee + sellFee.devfee + buyFee.marketingFee + sellFee.marketingFee + buyFee.charityFee + sellFee.charityFee + buyFee.buybackFee + sellFee.buybackFee);
        uint256 charityShare = newBalance.mul(buyFee.charityFee + sellFee.charityFee).div(buyFee.devfee + sellFee.devfee + buyFee.marketingFee + sellFee.marketingFee + buyFee.charityFee + sellFee.charityFee + buyFee.buybackFee + sellFee.buybackFee);
        uint256 buybackShare = newBalance.mul(buyFee.buybackFee + sellFee.buybackFee).div(buyFee.devfee + sellFee.devfee + buyFee.marketingFee + sellFee.marketingFee + buyFee.charityFee + sellFee.charityFee + buyFee.buybackFee + sellFee.buybackFee);
        uint256 devShare =newBalance.mul(buyFee.devfee + sellFee.devfee).div(buyFee.devfee + sellFee.devfee + buyFee.marketingFee + sellFee.marketingFee + buyFee.charityFee + sellFee.charityFee + buyFee.buybackFee + sellFee.buybackFee);
        IERC20(BUSD).transfer(_marketingWallet, marketingShare);
        IERC20(BUSD).transfer(_charityWallet, charityShare);
        IERC20(BUSD).transfer(_buybackWallet, buybackShare);
        IERC20(BUSD).transfer(_developmentWallet, devShare);
}
